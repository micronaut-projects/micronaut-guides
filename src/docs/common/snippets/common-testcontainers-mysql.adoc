...
:dependencies:
dependency:testcontainers-mysql[groupId=org.testcontainers,scope=testImplementation]
dependency:testcontainers[groupId=org.testcontainers,scope=testImplementation]
ifeval::["{testFramework}" == "junit"]
dependency:testcontainers-junit-jupiter[groupId=org.testcontainers,scope=testImplementation]
endif::[]
:dependencies:

Configure the default datasource to use the MySQL database provided by TestContainers for the test environment:

testResource:application-test.properties[tag=testcontainers]
