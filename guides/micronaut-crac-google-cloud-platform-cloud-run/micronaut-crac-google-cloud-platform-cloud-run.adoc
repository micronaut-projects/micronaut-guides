...
NOTE: In the image name above, `micronaut-guides-xxxxxxx` should be replaced by the project name you created earlier in the guide.
...
----
gcloud run deploy \
    --image=gcr.io/micronaut-guides-xxxxxxx/micronautguide:latest \
    --platform managed \
