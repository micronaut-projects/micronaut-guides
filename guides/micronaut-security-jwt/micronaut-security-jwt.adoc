...
Note the following configuration in the generated configuration file:
...
<2> You can specify the HTTP verb that a controller action responds to. To respond to a GET request, use the `io.micronaut.http.annotation.Get` annotation.
<3> The root route responds to `/`.
...
:exclude-for-languages:java,groovy,kotlin
<.> Use `requests.with_context` to send HTTP requests to the running test server.
:exclude-for-languages:

<.> When you include the security dependencies, security is considered enabled and every endpoint is secured by default.
<.> To login, do a POST request to `/login` with your credentials as a JSON payload in the body of the request.
:exclude-for-languages:python
<.> The framework makes it easy to bind JSON responses into Java objects.
:exclude-for-languages:
:exclude-for-languages:java,groovy,kotlin
<.> The `requests` response exposes JSON payloads as Python dictionaries.
:exclude-for-languages:
<.> The framework supports the https://tools.ietf.org/html/rfc6750[RFC 6750 Bearer Token] specification out-of-the-box. We supply the JWT token in the `Authorization` HTTP header.
:exclude-for-languages:python
callout:body-method[]
:exclude-for-languages:
:exclude-for-languages:java,groovy,kotlin
<.> The `requests` response exposes the response body through `text`.
:exclude-for-languages:
...
resource:application.toml[tag=refresh]
...
source:RefreshTokenEntity[tags=clazzwithoutsettersandgetters]
...
Create a https://micronaut-projects.github.io/micronaut-data/latest/api/io/micronaut/data/repository/CrudRepository.html[CrudRepository] to include methods to perform Create, Read, Update, and Delete operations with the `RefreshTokenEntity`.
...
callout:singleton[]
<.> Constructor injection of `RefreshTokenRepository`.
<.> When a new refresh token is issued, the application emits an event of type https://micronaut-projects.github.io/micronaut-security/latest/api/io/micronaut/security/token/event/RefreshTokenGeneratedEvent.html[RefreshTokenGeneratedEvent]. We listen for it and save the token in the database.
<.> The event contains both the refresh token and the user details associated to the token.
<.> Throw an exception if the token is revoked.
<.> Return the user details associated to the refresh token, e.g. username, roles, attributes, etc.
