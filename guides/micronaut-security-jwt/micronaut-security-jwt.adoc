...
Micronaut ships with security capabilities based on https://jwt.io/[Json Web Token (JWT)]. JWT is an https://tools.ietf.org/html/rfc7519[IETF standard] which defines a secure way to encapsulate arbitrary data that can be sent over unsecure URLs.
...
To access a secured endpoint, you can also use Micronaut's HTTP Client and supply the JWT token in the Authorization header.
