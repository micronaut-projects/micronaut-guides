...
=== Create an Application with the Command Line Interface
...
Add the Micronaut `security-jwt` dependency:
...
We may want to save a refresh token issued by the application. For example, to be revoked a user's refresh tokens. So that a particular user cannot obtain a new access token, and thus access the application's endpoints.
...
Enable the https://micronaut-projects.github.io/micronaut-security/latest/guide/index.html#refresh[Refresh Controller] via configuration and provide an implementation of
https://micronaut-projects.github.io/micronaut-security/latest/api/io/micronaut/security/token/refresh/RefreshTokenPersistence.html[RefreshTokenPersistence].
