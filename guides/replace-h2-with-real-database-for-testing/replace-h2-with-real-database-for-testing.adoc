...
We are going to create a sample Micronaut application using *Micronaut Data JDBC* and *PostgreSQL*.
...
First let us start with creating a Micronaut Data entity.
...
Let us create a Micronaut Data JDBC repository for the *Product* entity.
...
resource:application.toml[app=h2,tag=datasource]
:exclude-for-languages:
...
:exclude-for-languages:
:exclude-for-languages:java
callout:seed-db-pyronaut[]
:exclude-for-languages:

If you run this test, the application uses the https://www.h2database.com/html/main.html[H2] in-memory database.

The test initializes the table, loads the sample data required for our test, calls the repository method, and asserts the expected result.
...
When you execute the above query with https://www.h2database.com/html/main.html[H2], you will get the following exception:
...
:exclude-for-languages:
:exclude-for-languages:java
resource:application.toml[app=postgresql,tag=datasource]
:exclude-for-languages:

We disable schema generation with `datasources.default.schema-generate=none`. The datasource allows up to 60 seconds for PostgreSQL to accept connections before Hikari fails initialization. We will use a https://java.testcontainers.org/modules/databases/jdbc/#using-a-classpath-init-script[classpath init script with Testcontainers] instead to load the following SQL file.
...
dependency:testcontainers-postgresql[groupId=org.testcontainers,scope=test]
...
:exclude-for-languages:java
testResource:application-test.toml[app=postgresql,tag=testcontainers-jdbc-url]
...
We can also indicate which version of the PostgreSQL database to use by specifying the Docker image tag after *postgresql* as follows:
...
The special JDBC URL also works for other databases such as *MySQL*, *PostGIS*, *YugabyteDB*, and *CockroachDB*.

:exclude-for-languages:python
=== Initializing the database container using Testcontainers and JUnit

If using a special JDBC URL doesn't meet your needs, or you need more control over the container creation,
...
https://micronaut-projects.github.io/micronaut-data/latest/guide/#jdbc[Micronaut Data JDBC] requires you
...
== Summary
We have looked at how to test Micronaut Data JDBC repositories using the https://www.h2database.com/html/main.html[H2] in-memory database and talked about
the drawbacks of using different (in-memory) databases for testing while using a different type of database
in production.

Then we learned how to replace the https://www.h2database.com/html/main.html[H2] database with a real database for testing using
Testcontainers special JDBC URL.
:exclude-for-languages:python
We also looked at using Testcontainers JUnit 5 extension annotations to spin up the database for testing which gives more control over the lifecycle of the database container.
:exclude-for-languages:
