...
You are going to deploy a Micronaut application as a HTTP Function to https://cloud.google.com/functions[Google Cloud Functions].

____
Cloud Functions is a serverless execution environment for building and connecting cloud services. With Cloud Functions you write simple, single-purpose functions that are attached to events emitted from your cloud infrastructure and services. Your function is triggered when an event being watched is fired.
____
include::{commondir}/common-requirements-jdk11.adoc[]
...
The entry point of your HTTP Cloud Function is `io.micronaut.gcp.function.http.HttpFunction`. That it is a class provided by https://micronaut-projects.github.io/micronaut-gcp/latest/guide/#httpFunctions[Micronaut GCP Function HTTP] dependency. This dependency allows developers to create applications triggered by HTTP requests with   classes annotated like `@Controller` and `@Get` etc.

That it is to say, developers code HTTP Server applications deployed to HTTP Cloud Functions the same way as if they were targeting a runtime such as https://netty.io[Netty]. Micronaut GCP abstracts developers of the Cloud Function runtime and let them focus on their code.
...
The previous snippet exposes two endpoints a `GET` and `POST` endpoints in the `/micronautguide` path.
...
include::{commondir}/common-google-cloud-platform.adoc[]

include::{commondir}/common-google-cloud-cli.adoc[]
...
You need a single JAR. Delete every jar but `micronautguide-0.1.jar`.
...
You need a single JAR. Delete every jar but `*.all-jar`.
