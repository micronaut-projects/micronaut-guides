...
* AWS Secrets Manager client needs a Sdk HTTP Client bean to bea available at bootstrap.

== Create Secret

OAuth 2.0 clients have a client id and secret property. We are going to save both in AWS Secrets Manager.

Createa secret in AWS Secrets Manager
