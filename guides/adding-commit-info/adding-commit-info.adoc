...
NOTE: The .zip file contains the completed project which is
almost ready to run.  The project aims to demonstrate how to
provide Git commit information to the `/info` endpoint and in
order for that to work the project needs to be in a Git
repository.  A simple way to create a local Git repository for
the downloaded project is to unzip the file and execute
`git init` in the root directory of the extracted project.
