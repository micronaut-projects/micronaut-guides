...
NOTE: Before running the downloaded project, follow the steps described in
the *Initialize Git Repository* section below.
...
`/info` endpoint and in order for that to work the project needs to be in a Git
repository.  After creating the project, initialize a Git repository from the root
...
annotate a class so the Graal visitor in charge of generating the GraalVM `resource-config.json` file is triggered:

:exclude-for-languages:

:exclude-for-languages:kotlin,groovy
...
[source, java]
.src/main/java/example/micronaut/Application.kt
----
package example.micronaut
...
// Using inline code instead of resource macro
// until resource becomes aware of the surrounding
// :exclude-for-languages
[source, json]
