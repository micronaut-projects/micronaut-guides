...
In this guide, you will add Git commit info to your Micronaut build artifacts and running application. There are many benefits to keeping your commit info handy:
...
`/info` endpoint. For that to work, the project needs to be in a Git repository. After creating the project, initialize a Git repository from the root of the newly created project:
...
To use the management features described in this section, add the dependency to your classpath.
...
If a `git.properties` file is available on the classpath, the https://docs.micronaut.io/latest/api/io/micronaut/management/endpoint/info/source/GitInfoSource.html[GitInfoSource] will expose the values in that file under the `git` key. Configure the generation of a `git.properties` file as part of your build.
...
For the Pyronaut example, use a small Python build helper to generate `git.properties` from the local Git repository and write it to the resource directory:

source:GenerateGitProperties[]

The Python `setup.py` build command can call the same helper before packaging so the generated `git.properties` file is available on the classpath.

:exclude-for-languages:

== Test

Create a test to verify that when you make a GET request to `/info` you get a payload such as:
...
Create a test to verify the behaviour:
...
will be Micronaut beans defined (something annotated with `@Singleton`, `@Controller`,...), but for this case you need to
