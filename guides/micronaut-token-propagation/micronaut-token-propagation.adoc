...
[source,bash]
----
mn create-app example.micronaut.gateway --build=@build@ --lang=@lang@
----
...
Create a tests to verify the application is secured and we can access it after login:
...
[source,bash]
----
mn create-app example.micronaut.userecho --build=@build@ --lang=@lang@
----
...
<1> Configure a fix port where the application listens.
...
In an application with several controllers and declarative clients, it can lead to a lot of repetition. Fortunately, Micronaut
includes a feature called token propagation. We can tell our application to propagate the incoming token to a set of outgoing requests.
...
and https://micronaut-projects.github.io/micronaut-security/latest/guide/index.html[Security] inside Micronaut.
