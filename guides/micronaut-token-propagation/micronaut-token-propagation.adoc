...
<.> The `@Client` annotation is used with a service id. We will reference the exact service id in the configuration shortly.
callout:requires-not-env-test[]
<.> Use `@Get` annotation to define the client mapping
<.> Supply the JWT to the HTTP Authorization header value to the `@Client` method.

Add this snippet to the application configuration to configure the service URL of the `echo` service.
...
resource:application.toml[app=intermediate-gateway,tag=httpservices]
...
Add this snippet to the application configuration to configure security:
...
resource:application.toml[app=intermediate-gateway,tag=security]
...
:exclude-for-languages:
:exclude-for-languages:java,groovy,kotlin
Create a route that exposes the `/user` endpoint.
:exclude-for-languages:
...
Add this snippet to the application configuration to change the port where `userecho` starts:
...
resource:application.toml[app=intermediate-userecho,tag=port]

:exclude-for-languages:

<1> Configure the port where the application listens.

Add this snippet to the application configuration:
...
resource:application.toml[app=intermediate-userecho,tag=security]

:exclude-for-languages:

<1> You can create a `SecretSignatureConfiguration` named `validation` that can validate JWTs generated by the `gateway` microservice.
...
Let's configure token propagation. We need to modify the application configuration in the `gateway` microservice:
...
resource:application.toml[app=gateway,tag=tokenpropagation]
...
:exclude-for-languages:
:exclude-for-languages:java,groovy,kotlin
Edit `user_controller.py` and remove the `@Header` parameter:
:exclude-for-languages:
...
:exclude-for-languages:
:exclude-for-languages:java,groovy,kotlin
Edit `username_fetcher.py` and remove the `authorization` parameter:
:exclude-for-languages:
...
:exclude-for-languages:
:exclude-for-languages:java,groovy,kotlin
Edit `user_echo_client.py` and remove the `@Header` parameter:
:exclude-for-languages:
...
:exclude-for-languages:
:exclude-for-languages:java,groovy,kotlin
Edit `test_user_echo_client_replacement.py` and remove the `authorization` parameter:
:exclude-for-languages:
