...
We generate a valid JWT in the gateway microservice. Then every microservice in our app is able to validate this JWT. We want every internal request to contain a valid JWT token. If we want to talk to another microservice we need to propagate the valid JWT get received.
...
We are going to write the app first without token propagation. Then we are going to configure a token propagation, and you will see how much code we can get rid of.
