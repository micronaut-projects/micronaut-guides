...
from micronaut.objectstorage.googlecloud import (
...
class ProfilePicturesController:
    ...
    def __init__(
        self,
        object_storage: GoogleCloudStorageOperations,  # <3>
...
# end::begin-class[]
    ...
    def upload(
        ...
        response = self.object_storage.upload(object_storage_upload)  # <3>

        return (
            HttpResponse.created(self.location(request, userId))  # <4>
    ...
    @staticmethod
    def build_streamed_file(entry: GoogleCloudStorageEntry) -> HttpResponse[StreamedFile]:
        native_entry = entry.getNativeEntry()
        content_type = native_entry.getContentType()
        media_type = (
            MediaType.of(content_type)
            if content_type is not None
            else MediaType.APPLICATION_OCTET_STREAM_TYPE
        )
        file = StreamedFile(entry.getInputStream(), media_type).attach(entry.getKey())
        http_response = HttpResponse.ok().header(HttpHeaders.ETAG, native_entry.getEtag())  # <3>
