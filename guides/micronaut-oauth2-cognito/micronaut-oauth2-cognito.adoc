...
Configure the appropriate callback, make sure you allow the `Authorization code grant` OAuth flow, and select the `openid` and `email` scopes.
...
:exclude-for-languages:python
Create a controller to handle requests to `/`. You will display the email of the authenticated person if any. Annotate the controller endpoint with `@View` since we will use a Thymeleaf template.
:exclude-for-languages:
:exclude-for-languages:java,groovy,kotlin
Create a route function to handle requests to `/`. You will display the email of the authenticated person if any. Annotate the route with `@View` since we will use a Thymeleaf template.
:exclude-for-languages:
