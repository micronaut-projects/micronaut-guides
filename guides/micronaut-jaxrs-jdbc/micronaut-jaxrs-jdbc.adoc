...
Create a dataclass to encapsulate the HTTP request body for a save request:
...
<.> A JAX-RS annotation to define the base URI for methods in this class.
callout:constructor-di[arg0=PetRepository]
<.> A JAX-RS annotation to define an endpoint for HTTP Get requests. Mapped to the base URI `/pets`, since no `@Path` is provided.
<.> The response is converted to a JSON array automatically.
<.> Paths can be templated for path parameters, such as the pet's name, here.
<.> Associate the template placeholder name with the Java variable.
...
callout:http-client[]
<.> Micronaut returns a 404 response if the resource method returns an empty optional.
...
resource:application-prod.properties[tag=prod-datasource]

:exclude-for-languages:

:exclude-for-languages:java,groovy,kotlin

resource:application-prod.toml[tag=prod-datasource]
