...
In this guide, we push a Docker image of a Micronaut application to https://www.oracle.com/cloud/cloud-native/container-registry/[Oracle Cloud Container Registry].
...
The following code sample uses the Gradle Kotlin DSL. You will need to set the environment variable `REGISTRY_TOKEN` to the auth token you created previously, and set the OCI email address, region, and namespace you obtained earlier.
...
You can push to Oracle Cloud Container Registry via the `dockerPush` Gradle task.
...
image-name = "ocir.eu-madrid-1.oci.oraclecloud.com/bzcsmyxwoeez/micronaut-guides"
----
...
[source,bash]
----
echo "${REGISTRY_TOKEN}" | docker login ocir.eu-madrid-1.oci.oraclecloud.com -u "bzcsmyxwoeez/email@example.com" --password-stdin
----
...
docker push ocir.eu-madrid-1.oci.oraclecloud.com/bzcsmyxwoeez/micronaut-guides:1.0.0
----
