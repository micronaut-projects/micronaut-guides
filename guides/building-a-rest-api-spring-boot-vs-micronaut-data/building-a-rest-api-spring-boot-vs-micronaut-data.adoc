...
dependency:spring-boot-starter-data-jdbc[groupId=org.springframework.boot]
...
We modified the `SaasSubscription` type. We use it as a persistence entity.
...
However, to keep applications as close as possible, you will create the database schema with an SQL file in the Micronaut application. https://micronaut-projects.github.io/micronaut-test/4.3.0/guide/#sql[Micronaut Test supports loading SQL before tests] seamlessly.
