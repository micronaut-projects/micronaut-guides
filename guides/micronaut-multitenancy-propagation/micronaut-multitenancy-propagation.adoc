...
<3> In the gateway we use the https://docs.micronaut.io/latest/api/io/micronaut/multitenancy/tenantresolver/CookieTenantResolver.html[CookieTenantResolver^] which resolves the current tenant from an HTTP cookie.
<4> We propagate the tenant with a https://docs.micronaut.io/latest/api/io/micronaut/multitenancy/writer/HttpHeaderTenantWriter.html[HttpHeaderTenantWriter^] which writes the current tenant to a HTTP Header.
...
The `views` module provides support for view rendering on the server side and does so by rendering views on the I/O thread pool in order to avoid blocking the Netty event loop. Add the `views` dependency:
...
You can visit `http://localhost:8080[http://localhost:8080^]` and change tenant and see the book list change:

image:multitenancy.gif[]

== Next Steps

Read more about https://docs.micronaut.io/latest/guide/#multitenancy[Multitenancy^] support in Micronaut framework and https://gorm.grails.org/latest/hibernate/manual/#multiTenancy[GORM Multitenancy support^].
