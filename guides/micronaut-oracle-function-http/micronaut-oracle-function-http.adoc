...
This will be a unit test, so we'll need some mock beans to replace the beans auto-registered by the `oracle-cloud-sdk` module which make requests to Oracle Cloud. We also need a workaround for the strict classloader isolation used by https://fnproject.io/[Fn Project] (which powers Oracle Cloud Functions).
...
<1> The `AuthCachingPolicy` annotation is used to disable caching; without this we would need to provide a valid private key since the provider methods would be invoked when constructing SDK client classes
<2> We use `@Replaces` to replace a previously auto-registered `ConfigFileAuthenticationDetailsProvider` bean (which happens if $HOME/.oci/config exists)
