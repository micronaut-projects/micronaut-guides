...
[source,bash]
----
mn create-app --features=oracle-function,oracle-cloud-sdk example.micronaut.micronautguide --build=@build@ --lang=@lang@ --jdk=11
----

include::{commondir}/common-build-lang-arguments.adoc[]

If you use https://launch.micronaut.io[Micronaut Launch], select "Micronaut Application" as application type, JDK version 11 or higher, and add the `oracle-function` and `oracle-cloud-sdk` features.
...
NOTE: The code in this guide should work with recent versions of Micronaut Framework 2.5+, but the tests require at least version 2.5.7
