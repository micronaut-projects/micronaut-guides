...
<1> A dependency on `micronaut-data` Reactive Hibernate support.
<2> Add a dependency to the Vert.x MySQL client.
...
<1> Configure the package that contains your entity classes.
...
The repository extends `ReactorPageableRepository`. It inherits the hierarchy `ReactorPageableRepository` -> `ReactorCrudRepository` -> `GenericRepository`.
...
<4> Returning an empty optional when the genre doesn't exist makes Micronaut Framework respond with 404 (not found).
...
<5> If you care only about the object in the response, use `retrieve`.
<6> Sometimes, receiving just the object is not enough, and you need information about the response. In this case, instead of `retrieve`, use the `exchange` method.
...
This can be done with environment variables like so:
