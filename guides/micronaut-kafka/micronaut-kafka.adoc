...
<.> The @api@/io/micronaut/http/annotation/Controller.html[@Controller] annotation defines the class as a controller mapped to the root URI `/books`
...
<.> Dependency injection for the `AnalyticsListener` class declared below, a Kafka listener class that replicates the functionality of the class of the same name in the `analytics` microservice
<.> Dependency injection for an HTTP client that the Micronaut framework will implement at compile time to make calls to `BookController`
:exclude-for-languages:java,groovy
callout:atfield[]
callout:lateinit[]
<.> Use the `HttpClient` to retrieve a `Book`, which will trigger sending a message with Kafka
...
<.> Verify that the message was received and has the correct data
<.> Wait a few seconds to make sure no message is sent
:exclude-for-languages:
...
<5> The filter is annotated with `@Blocking` because it sends a Kafka message
