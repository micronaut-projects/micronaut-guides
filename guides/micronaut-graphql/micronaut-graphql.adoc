...
Create the file `schema.graphqls` in the `config` directory:
...
resource:application.toml[tag=jackson]
...
For testing the application we will use a context-bound Pyronaut requests client to send a `POST` request to the `/graphql` endpoint.
