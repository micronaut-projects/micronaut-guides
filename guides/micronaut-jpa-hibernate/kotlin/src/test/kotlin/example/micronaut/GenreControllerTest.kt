...
class GenreControllerTest {
    ...
    fun testGenreCrudOperations() {
        ...
        var request: HttpRequest<*> = HttpRequest.POST("/genres", GenreSaveCommand("DevOps")) // <3>
        ...
        request = HttpRequest.POST("/genres", GenreSaveCommand("Microservices")) // <3>
        ...
        var genre = blockingClient!!.retrieve(request, Genre::class.java) // <4>
        ...
        request = HttpRequest.POST("/genres/ex", GenreSaveCommand("Microservices")) // <3>
        response = blockingClient!!.exchange(request)
        Assertions.assertEquals(HttpStatus.NO_CONTENT, response.status)
