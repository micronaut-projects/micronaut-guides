...
NOTE: This way of defining the datasource properties means that we can externalize the configuration, for example for production environment, and also provide a default value for development. If the environment variables are not defined Micronaut will use the default values. +
...
In Micronaut, like Spring Boot and Grails, you can create type safe configuration by creating classes that are annotated with https://docs.micronaut.io/latest/guide/index.html#configurationProperties[@ConfigurationProperties].
