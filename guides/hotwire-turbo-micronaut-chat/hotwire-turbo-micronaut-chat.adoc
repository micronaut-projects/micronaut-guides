...
<.> `TurboMediaType::acceptsTurboStream` is a convenient method to verify if the request accepts a turbo stream response.
...
resource:application.toml[tag=datasource,app=initial]
...
resource:application.toml[tag=flyway,app=initial]
...
resource:application.toml[tag=static-resources,app=initial]
