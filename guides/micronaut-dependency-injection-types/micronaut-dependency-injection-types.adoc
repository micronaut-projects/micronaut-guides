...
Constructor injection keeps dependencies assigned when the object is created, so the instance starts with its required collaborators.
:exclude-for-languages:
...
Thus, it decreases the chance of runtime failures during testing.
