...
Create a controller that you can invoke once the application is deployed.
...
To see the log of your application running on Azure, first enable storage for application logging, and then tail the log for live streaming.

To enable logging storage, use the following Azure CLI command. Check your `pom.xml` for the generated `<resourceGroup>` and `<appName>` values.
...
If you don't see any log output, try changing the level of the project's Logback configuration in `logback.xml` to a higher value such as `debug`, then run the Maven deployment command again (you may need to run `mvn clean` before this step):
...
After you finish this guide, you can clean up the resources you created on
Azure so you won't risk being billed for them in the future. The following
sections describe how to delete or turn off these resources.

Delete the resource group and all of the resources created during this guide with the following command:
