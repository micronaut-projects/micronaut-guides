...
main method, or use `mvn spring-boot:run` or `./gradlew bootRun`). It will start up on port 8761 and serve the Eureka
API from `/eureka`.
...
Previous configuration registers a Micronaut app with Eureka with minimal configuration. Discover a more complete list
of Configuration options at https://docs.micronaut.io/latest/api/io/micronaut/discovery/eureka/EurekaConfiguration.html[EurekaConfiguration].
