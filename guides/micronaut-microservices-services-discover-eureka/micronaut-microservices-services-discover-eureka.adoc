...
[source,bash]
----
mn @bookcatalogue:cli-command@ --features=discovery-eureka,management,graalvm example.micronaut.bookcatalogue --build=@build@ --lang=@lang@
----

common:build-lang-arguments.adoc[]

If you use Micronaut Launch, select Micronaut Application as application type and add the `discovery-eureka`, `management`, and `graalvm` features.
...
<4> Easily parse JSON into Java objects.

Edit the `bookcatalogue` service configuration:
...
resource:application.toml[app=bookcatalogue,tag=mn]
:exclude-for-languages:
...
:exclude-for-languages:java,groovy,kotlin
resource:application.toml[app=bookcatalogue,tag=server]
:exclude-for-languages:

<1> Configure the application to listen on port 8081.

Create test configuration for the test environment:
...
[source,bash]
----
mn @bookinventory:cli-command@ --features=discovery-eureka,management,graalvm example.micronaut.bookinventory --build=@build@ --lang=@lang@
----
...
Edit the `bookinventory` service configuration:
...
:exclude-for-languages:java,groovy,kotlin
resource:application.toml[app=bookinventory,tag=mn]
:exclude-for-languages:
...
:exclude-for-languages:java,groovy,kotlin
resource:application.toml[app=bookinventory,tag=server]
:exclude-for-languages:
...
:exclude-for-languages:java,groovy,kotlin
testResource:application-test.toml[app=bookinventory]
:exclude-for-languages:
...
[source,bash]
----
mn @bookrecommendation:cli-command@ --features=discovery-eureka,management,reactor,graalvm example.micronaut.bookrecommendation --build=@build@ --lang=@lang@
----

common:build-lang-arguments.adoc[]

If you use Micronaut Launch, select Micronaut Application as application type and add the `discovery-eureka`, `management`, `reactor`, and `graalvm` features.
...
<1> Make this fallback class effective only when the Micronaut environment __TEST__ is active.
<2> Here we arbitrarily decide that if everything else fails, that book's `stock` would be true.
<3> Similarly, we decide that the other book's `stock` method would be false.
<4> Finally, any other book will have its `stock` method return an empty value.
...
Edit the `bookrecommendation` service configuration:
...
:exclude-for-languages:java,groovy,kotlin
resource:application.toml[app=bookrecommendation,tag=mn]
:exclude-for-languages:
...
:exclude-for-languages:java,groovy,kotlin
resource:application.toml[app=bookrecommendation,tag=server]
:exclude-for-languages:
...
:exclude-for-languages:java,groovy,kotlin
testResource:application-test.toml[app=bookrecommendation]
:exclude-for-languages:
...
Append to the `bookcatalogue` service configuration the following snippet:

:exclude-for-languages:python
resource:application.properties[app=bookcatalogue,tag=eureka]
:exclude-for-languages:

:exclude-for-languages:java,groovy,kotlin
resource:application.toml[app=bookcatalogue,tag=eureka]
:exclude-for-languages:

The previous configuration registers a Micronaut application with Eureka with minimal configuration. Discover a more complete list of configuration options at https://micronaut-projects.github.io/micronaut-discovery-client/latest/api/io/micronaut/discovery/eureka/EurekaConfiguration.html[EurekaConfiguration].

=== Book Inventory

Append the following snippet to the `bookinventory` service configuration:

:exclude-for-languages:python
resource:application.properties[app=bookinventory,tag=eureka]
:exclude-for-languages:

:exclude-for-languages:java,groovy,kotlin
resource:application.toml[app=bookinventory,tag=eureka]
:exclude-for-languages:

=== Book Recommendation

Append the following snippet to the `bookrecommendation` service configuration:

:exclude-for-languages:python
resource:application.properties[app=bookrecommendation,tag=eureka]
:exclude-for-languages:

:exclude-for-languages:java,groovy,kotlin
resource:application.toml[app=bookrecommendation,tag=eureka]
:exclude-for-languages:
...
:exclude-for-languages:java,groovy,kotlin
source:BookCatalogueClient[app=bookrecommendation]
:exclude-for-languages:
...
:exclude-for-languages:java,groovy,kotlin
source:BookInventoryClient[app=bookrecommendation]
:exclude-for-languages:
