...
NOTE: The Azure Key Vault secrets naming convention does not allow `_` to be used as a word joiner. Therefore, when adding JDBC secret placeholders, you should use `-` instead, such as `JDBC-USER` and `JDBC-PASSWORD`.
...
NOTE: The client's secret value cannot be viewed except immediately after creation. Be sure to save the secret before leaving the page.
{empty} +
