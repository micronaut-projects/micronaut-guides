...
We will define some environment variables to make deploying process easier. In `OCI_USER_ID` store your user OCI, which you can find in your oci configuration file. In `OCI_TENANCY_NAME` store your tenancy name. In `OCI_USERNAME` store your username in the format <tenancy>/<username>. We can reuse `OCI_TENANCY_NAME` and only edit the <username> part. Store your region key code in `OCI_REGION` for an example "us-phoenix-1". In `OCI_CLUSTER_ID` store the Cluster Id that you copied earlier.
