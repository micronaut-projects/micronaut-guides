...
Choose a name for the Kubernetes cluster. We chose `micronaut-k8s`.
...
We will define some environment variables to make deployment easier. In `OCI_USER_ID`, store your OCI user ID, which you can find in your OCI configuration file. We will get the tenancy namespace from the OCI command-line tool and store it in `OCI_TENANCY_NAMESPACE`. In `OCIR_USERNAME`, store your username in the format <tenancy_namespace>/<username>. We can reuse `OCI_TENANCY_NAMESPACE` and only edit the <username> part. Store your region code in `OCI_REGION`, for example, "us-phoenix-1". In `OCI_CLUSTER_ID`, store the Cluster ID that you copied earlier. Put your compartment ID inside the `OCI_COMPARTMENT_ID` variable.
...
NOTE: Oracle Cloud allows you to have only 2 auth tokens at the same time. If you already have 2 of them, use an existing one by exporting it in the AUTH_TOKEN variable or delete one that you are not using.
...
Create an `ocirsecret` secret that will be used for authentication to Oracle Cloud Container Registry. This is needed because OKE needs credentials to be able to pull microservice images.
...
NOTE: If `EXTERNAL-IP` is in the <pending> state, wait a couple of seconds and then run the command again.
...
Run the next command to delete the OKE cluster.
