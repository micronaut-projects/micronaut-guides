...
resource:application.toml[tag=metrics]
:exclude-for-languages:

<1> Change to `false` to disable all metrics, for example, per environment.
...
<.> Specifies that `Book` is the root entity, and the primary key type is `Long`.
...
callout:executes-on[]
callout:constructor-di[arg0=BookRepository]
<.> Maps a `GET` request to `/books`, which returns a list of `Book`
<.> Creates a `Timer` metric with the given name. If the name is omitted, defaults to `method.timed`.
<.> Maps a `GET` request to `/books/{isbn}`, which attempts to show a `Book`. This illustrates the use of a URL path variable.
<.> Creates a `Counter` metric with the given name. If the name is omitted, defaults to the `method.counted`.
...
Modify the application configuration to configure the URL for the `kucoin` Service ID:
...
:exclude-for-languages:java,groovy,kotlin
resource:application.toml[tag=http-services]
:exclude-for-languages:
...
callout:singleton[]
<.> Use constructor injection to inject `PriceClient` and `MeterRegistry` beans
<.> Create a `Counter` to track the total number of updates
<.> Create a `Timer` to track the total time performing updates
<.> Create a `Gauge` to track the most recent update
<.> Use the `Scheduled` annotation to configure regular updates
<.> Update the `Timer`
<.> Increment the `Counter`
<.> Update the `Gauge` with the latest price
...
:exclude-for-languages:java,groovy,kotlin
Create the `tests-config/application-test.toml` configuration file for tests.

testResource:application-test.toml[]
:exclude-for-languages:
...
<.> Inject the `MeterRegistry` bean
<.> Inject the `LoggingSystem` bean
callout:http-client[]
...
[source,bash]
----
pyronaut run -- -crypto.updateFrequency=5s
----
