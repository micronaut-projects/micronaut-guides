...
To learn more about this sample application read https://guides.micronaut.io/micronaut-microservices-services-discover-consul/guide/[Consul and the Micronaut framework - Microservices service discovery] guide. The application contains three microservices.
...
The Micronaut framework includes several annotations to give you more flexibility. We introduced the `@ContinueSpan`, `@SpanTag` annotations. Also, you have at your disposal the `@NewSpan` annotation which will create a new span, wrapping the method call or reactive type.

Make sure to read more about https://docs.micronaut.io/latest/guide/#jaeger[Tracing with Jaeger] in the Micronaut framework.
