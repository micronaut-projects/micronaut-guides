...
testResource:GebConfig.groovy[]

<1> Disable confirmation popups
<2> Configure the download folder

Create a Geb Page to encapsulate the Excel link:
...
[source,bash]
----
$ ./gradlew -Dgeb.env=chrome -Ddownload.folder=/tmp test
