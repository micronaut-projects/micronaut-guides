...
When you start the Micronaut applications, it fetches the Auth0 application's `openidconfiguration`:
...
The previous configuration uses several placeholders. You will need to setup `OAUTH_CLIENT_ID`, `OAUTH_CLIENT_SECRET` and `OAUTH_DOMAIN` environment variables.
