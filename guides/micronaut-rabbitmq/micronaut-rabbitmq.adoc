...
<1> Dependency injection for `BookService`.
<2> Expose a route to return all books.
<3> Expose a route to return a book by ISBN.
:exclude-for-languages:
...
:exclude-for-languages:
:exclude-for-languages:java,groovy,kotlin
<1> Create a Pyronaut test fixture with `@MicronautTest`.
<2> Resolve the `AnalyticsService` bean from the test context.
:exclude-for-languages:
...
resource:application-dev.properties[app=analytics]

<1> Start the `analytics` microservice on port 8081.
:exclude-for-languages:
:exclude-for-languages:java,groovy,kotlin
Create `config/application-dev.toml`. The Micronaut framework applies this configuration file only for the `dev` environment.
The configuration starts the `analytics` microservice on port 8081.

resource:application-dev.toml[app=analytics]
:exclude-for-languages:
...
Via <<test-resources,Test Resources>>, the Micronaut application will connect to a RabbitMQ instance running inside Docker so it is not necessary to add anything to `application.properties`.
In case you want to change the configuration, add the following:

:exclude-for-languages:python
resource:application-prod.properties[app=books,tag=rabbitmq]
:exclude-for-languages:
:exclude-for-languages:java,groovy,kotlin
resource:application-prod.toml[app=books,tag=rabbitmq]
:exclude-for-languages:
...
One option is to create them directly in the RabbitMQ Admin UI available on port 15672.
...
:exclude-for-languages:
:exclude-for-languages:java,groovy,kotlin
<1> Annotate the class with `@ServerFilter` and define the Ant-style matcher pattern to intercept all calls to the desired URIs.
<2> The filter uses an annotation-based `@ResponseFilter` method instead of implementing `HttpServerFilter`.
<3> Dependency injection for the RabbitMQ `AnalyticsClient`.
<4> The `@ResponseFilter` method runs after the request completes.
<5> The filter is annotated with `@ExecuteOn(TaskExecutors.BLOCKING)` because it sends a RabbitMQ message.
<6> Get the response body from the controller.
<7> If the book is found, use the RabbitMQ client to send a message.
:exclude-for-languages:
...
<1> Do not load this bean for the test environment. This enables us to run the tests without having a RabbitMQ instance running.
