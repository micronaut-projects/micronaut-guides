...
[source,bash]
----
mn create-app --features=rabbitmq,reactor,graalvm example.micronaut.books --build=@build@ --lang=@lang@
----
...
[source,bash]
----
mn create-app --features=rabbitmq,graalvm example.micronaut.analytics --build=@build@ --lang=@lang@
----
...
<1> Starting with Micronaut 1.1.0 `micronaut-test-junit5` is added automatically to `build.gradle` (or `pom.xml`) when creating an application with the CLI. For more information take a look https://micronaut-projects.github.io/micronaut-test/latest/guide/[at the documentation].
...
One option is create them directly in the RabbitMQ Admin UI available on `http://localhost:15672[http://localhost:15672]`.Use `guest` for both
...
Open RabbitMQ Admin UI on `http://localhost:15672[http://localhost:15672]` and use `guest` for both username and password. Select `queues` and
