...
First, create *Album* and *Photo* models using Python dataclasses.
:exclude-for-languages:
...
:exclude-for-languages:
:exclude-for-languages:java
So, let us add the following property in the *config/application.toml* file.

resource:application.toml[]
:exclude-for-languages:
...
<.> We can create an instance of WireMock server using *WireMockExtension* .
...
:exclude-for-languages:
:exclude-for-languages:java
<.> Create and stop a WireMock server for each test.
<.> Register the *"micronaut.http.services.photosapi.url"* property pointing to the WireMock server URL.
<.> In the *test_should_get_album_by_id()* test, set the expected mock response for `/albums/\{albumId}/photos`.
<.> Set the expectations for an API call.
<.> Make a request to our application endpoint `/api/albums/\{albumId}` and verify the response.
<.> In the *test_should_return_server_error_when_photo_service_call_failed()* test, set the expected mock response for `/albums/\{albumId}/photos` to return Internal Server Error status code 500 and verify the response.
:exclude-for-languages:
...
callout:test-containers-disabled-without-docker[]
<.> We are using Testcontainers JUnit 5 Extension annotations *@Container* to initialize *WireMockContainer*.
...
dependency:testcontainers-mockserver[groupId=org.testcontainers,scope=test]
...
<.> We have registered the *"micronaut.http.services.photosapi.url"* property pointing to MockServer container endpoint.
callout:injection-request-specification[]
callout:rest-assured-spec-port[]
:exclude-for-languages:
:exclude-for-languages:java
<.> Start a MockServer Testcontainers container for the test module.
<.> Register the *"micronaut.http.services.photosapi.url"* property pointing to MockServer container endpoint.
<.> Configure the application HTTP client to use the MockServer endpoint.
<.> In the *test_should_get_album_by_id()* test, create the MockServer expectation for `/albums/\{albumId}/photos`.
<.> Make a request to our application endpoint `/api/albums/\{albumId}` and verify the response.
:exclude-for-languages:
...
:exclude-for-languages:
:exclude-for-languages:java
and test it using https://wiremock.org[WireMock] or https://www.mock-server.com/[MockServer].
:exclude-for-languages:
