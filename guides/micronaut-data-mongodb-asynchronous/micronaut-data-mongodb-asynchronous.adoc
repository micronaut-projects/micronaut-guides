...
:exclude-for-languages:java,python
...
Create a repository to encapsulate the CRUD actions for `Fruit`.
...
callout:micronaut-test-transactional-false[]
<.> Clean up the database after each test method execution.
...
Create tests that verify validation and that the controller works against a real MongoDB database:

test:fruit_controller[]

callout:micronaut-test-transactional-false[]
<.> Clean up the database after each test method execution.

Create a test which uses a replacement `FruitService` to test the controller without touching the database:

test:controller_isolation[]
