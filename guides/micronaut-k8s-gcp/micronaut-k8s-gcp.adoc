...
Create the `micronaut-k8s` Kubernetes cluster:
...
Create a secret named `gcpsersecret` that will be used for authentication to Google Cloud Container Registry. This is needed because GKE needs credentials to be able to pull microservice images.
[source,bash]
----
