...
callout:singleton[]
<.> Specify that `DefaultAuthorizationExceptionHandlerReplacement` replaces the bean `DefaultAuthorizationExceptionHandler`
...
<.> By using `basicAuth` method, you populate the `Authorization` header with user-id:password pairs, encoded using Base64.

:exclude-for-languages:python
<.> The Micronaut `HttpClient` simplifies parsing the HTTP response payload to Java objects. In this example, we parse the response to `String`.
callout:body-method[]
...
callout:micronaut-test[]
<.> Inject the `AppClient` bean.
<.> Generate Basic Auth header value and pass it as the parameter value.
