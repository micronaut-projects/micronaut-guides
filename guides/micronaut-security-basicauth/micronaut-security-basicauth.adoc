...
Add Micronaut's security dependency to your build:
...
<6> Micronaut's `HttpClient` simplifies parsing the HTTP response payload to Java objects. In this example, we parse the response to `String`.
...
<1> The method consumes plain text, so Micronaut includes the HTTP Header `Accept: text/plain`.
