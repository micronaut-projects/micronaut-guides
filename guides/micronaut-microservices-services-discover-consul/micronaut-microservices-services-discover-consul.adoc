...
<.> Easily parse JSON into Java objects.
...
<.> By default, `Content-Type` of a controller response is `application/json` : override this with `text/plain` since we are returning a String, not a JSON object.
...
callout:controller[arg0=/books]
<.> Clients are injected via constructor injection
callout:get[arg0=index,arg1=/books]
...
Append the following snippet to the `bookcatalogue` service configuration:
...
:exclude-for-languages:java,groovy,kotlin
resource:application.toml[app=bookcatalogue,tag=consul]
:exclude-for-languages:
...
Modify the `bookinventory` application configuration with the following snippet:
...
:exclude-for-languages:java,groovy,kotlin
resource:application.toml[app=bookinventory,tag=consul]
:exclude-for-languages:
...
:exclude-for-languages:java,groovy,kotlin
resource:application.toml[app=bookrecommendation,tag=consul]
:exclude-for-languages:
