...
To use DynamoDB with Micronaut, you application should have the following dependencies:
...
Create a singleton class to handle common operation with DynamoDB.
...
Create an environment specific configuration class for the `dev` environment which defines the host and port of the DynamoDB Local instance.

resource:application-dev.yml[]

==== Dev Bootstrap

Create a `StartupEventListener` only loaded for the `dev` environment which creates the dynamodb table if it does not exists.
