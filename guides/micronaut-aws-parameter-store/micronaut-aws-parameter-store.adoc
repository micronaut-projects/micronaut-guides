...
<1> Set `aws.client.system-manager.parameterstore.enabled` to enable distributed configuration with AWS Parameter Store.

NOTE: The `micronaut.application.name` value will be part of the parameter name in AWS Parameter Store.
