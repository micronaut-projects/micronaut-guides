...
Create a controller that enables you to send POST requests to log messages:

source:LogController[]

== Create Azure Cloud Resources

You will create a resource group, a Log Analytics Workspace, a table in the workspace to hold log event data, a Data Collection Endpoint, and a Data Collection Rule.
...
Since the Azure appender queues log messages and then writes them remotely, there are situations that might result in log events not getting sent correctly.
...
Note that it can take a few minutes for log entries to be available.
