...
class SaasSubscriptionController {
    ...
    HttpResponse<SaasSubscription> findById(@PathVariable Long id, // <5>
        ...
        Optional<SaasSubscription> subscription = repository.findByIdAndOwner(id, principal.name)
        subscription.isPresent() ? HttpResponse.ok(subscription.get()) : HttpResponse.notFound() // <7>
    }
}
