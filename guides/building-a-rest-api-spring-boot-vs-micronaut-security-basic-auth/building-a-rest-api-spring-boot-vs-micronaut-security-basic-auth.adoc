...
In the Spring Boot application, add the https://docs.spring.io/spring-security/reference/getting-spring-security.html[Spring Boot Starter Security] dependency:
...
Security configuration differs between the frameworks, but the coding experience of accessing the authenticated user as a controller method parameter of type `java.security.Principal` is identical.
