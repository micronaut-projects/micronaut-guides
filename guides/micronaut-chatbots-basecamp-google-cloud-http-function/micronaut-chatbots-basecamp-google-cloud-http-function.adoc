...
You are going to deploy a https://github.com/basecamp/bc3-api/blob/master/sections/chatbots.md[Basecamp Chatbot] built with https://micronaut-projects.github.io/micronaut-chatbots/snapshot/guide/[Micronaut Chatbots] as an HTTP Function to https://cloud.google.com/functions[Google Cloud Functions].
____
...
mn @cli-command@ example.micronaut.micronautguide \
              --features=chatbots-basecamp-gcp-function \
              --build=@build@ \
              --lang=@lang@ \
              --jdk=11
----
...
By supplying the feature `chatbots-basecamp-gcp-function` to the CLI command `create-app`, the https://github.com/micronaut-projects/micronaut-gradle-plugin#micronaut-runtimes[Micronaut runtime] is set to `google_function`.
...
By supplying the feature `chatbots-basecamp-gcp-function` to the CLI command `create-app`, `Micronaut runtime` is set to `google_function`.
...
The entry point of your HTTP Cloud Function is `io.micronaut.chatbots.basecamp.googlecloud.Handler`. This is a class provided by the https://micronaut-projects.github.io/micronaut-gcp/latest/guide/#httpFunctions[Micronaut GCP Function HTTP] dependency. This dependency allows developers to create applications triggered by HTTP requests with classes annotated like `@Controller` and `@Get` etc.
...
gcloud functions deploy micronautguide
    --entry-point io.micronaut.chatbots.basecamp.googlecloud.Handler
    --runtime java11
    --trigger-http
----
...
- https://github.com/basecamp/bc3-api/blob/master/sections/chatbots.md[Basecamp Chatbot]
- https://micronaut-projects.github.io/micronaut-chatbots/snapshot/guide/[Micronaut Chatbots]
