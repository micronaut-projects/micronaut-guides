...
In this guide, you will use Vault to store database connection information for a Micronaut application that uses MySQL, and use Micronaut support for Oracle Cloud to make the process seamless.

== The Application

Download the complete solution for the guideLink:micronaut-data-jdbc-repository[Access a database with Micronaut Data JDBC] guide. You will use the sample application as a starting point.

== Creating an Oracle Cloud Vault

You will create the vault with the Oracle Cloud CLI. See the https://docs.oracle.com/en-us/iaas/tools/oci-cli/latest/oci_cli_docs/cmdref/kms.html[Vault CLI command reference] for more information.
...
Next, https://docs.oracle.com/en-us/iaas/Content/KeyManagement/Tasks/managingkeys.htm[create a master encryption key].

For this, you need the management endpoint URL that was not available yet in the response from the vault `create` command. Run the https://docs.oracle.com/en-us/iaas/tools/oci-cli/latest/oci_cli_docs/cmdref/kms/management/vault/get.html[get command]:
...
<1> Use https://micronaut-projects.github.io/micronaut-oracle-cloud/latest/guide/#instance-principals[Instance Principal authentication] to allow the Micronaut application to access Vault.
...
Use the guideLink:micronaut-oracle-cloud[Deploy a Micronaut application to Oracle Cloud] guide to create a compute instance and deploy the application to it; follow the steps in the "Create an Oracle Cloud Compute Instance" and "Deploy to Oracle Cloud" sections up to the step where you start the application. You need to connect the application to the MySQL database before starting it.

NOTE: When creating the compute VM at Oracle Cloud, use the same subnet where you created the MySQL database; otherwise, the application will not be able to access the database.
...
The guide recommends that you set environment variables to override the default values, but in this guide, you will go a step further and store some of those values in Oracle Cloud Vault. Leave the default for the driver class, but create vault secrets for `JDBC_USER`, `JDBC_PASSWORD`, and `JDBC_URL`.
...
Use https://micronaut-projects.github.io/micronaut-oracle-cloud/latest/guide/#instance-principals[Instance Principal authentication] to allow the Micronaut application to retrieve secrets from Vault. To use this, create a dynamic group and add a policy statement granting permission.
...
Choose a group name, e.g., "mn-guide-dg", and a matching rule, i.e., the logic that will be used to determine group membership. Make the rule fairly broad: use `ALL {instance.compartment.id = 'ocid1.compartment.oc1..aaaaaaaarkh3s2wcxbbm...'}` replacing `ocid1.compartment.oc1..aaaaaaaarkh3s2wcxbbm...` with the compartment OCID:
...
Create the policy in the root compartment, i.e., the tenancy, so use the tenancy OCID saved from the dynamic group creation response.
...
After you finish this guide, you can clean up the resources you created.
