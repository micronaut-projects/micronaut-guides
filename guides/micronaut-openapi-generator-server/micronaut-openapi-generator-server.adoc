...
* The search for a substring `"Gu"` in name will return a "Bad Request" error, since we have defined the name parameter to
...
We will implement tests for models inside these files. This is done to ensure that the generated model behaves as we expected.
