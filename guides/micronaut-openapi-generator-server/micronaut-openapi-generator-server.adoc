...
* You will learn to use the link:https://github.com/micronaut-projects/micronaut-openapi/[Micronaut OpenAPI] build tool plugins to generate Micronaut code in Java for the server application. We will extend the code by implementing internal logic and testing it.
...
<6> The `responses` object describes the response codes that can be produced. It also defines the structure of the body, if any.
<7> In the case of a correct request, we define the body to contain a list of `BookInfo` objects. The schema for the book info object will be defined later in the `components/schemas` section of the definition.
<8> The `"400"` status code will be produced by Micronaut in the case of a bad request, like an incorrect type supplied or failed validation. Even though Micronaut handles it automatically and no implementation is needed on our side, we add it for a complete API specification.
...
<2> We specify that a body is required and what content types it supports. In this case, only `application/json` is supported, but multiple content types can be allowed.
...
Schemas are required whenever a parameter, request body, or response body we want to describe needs to be an object. In that case, we add a schema that defines all the properties of the object. You can find out about the format for schemas in the link:https://oai.github.io/Documentation/specification-content.html["Content of Message Bodies" OpenAPI Guide].
...
----
<1> Loads the service definition to generate a server.
...
<2> It also generates model classes that are required by the service.
...
<3> It also generates model classes that are required by the service.
...
<.> `BookEntity`, the entity to treat as the root entity for the purposes of querying, is established either from the method signature or from the generic type parameter specified to the `CrudRepository` interface.
<.> The `save` method will be used to store new entities in the database
<.> Micronaut Data will generate the implementation of `findAllByAuthorContains` for us
<.> We can create as many finders as needed
<.> We can combine criteria in a single method name.
...
callout:executes-on[]
<.> Define the `searchEntities` method that will manage the different combinations of desired search parameters.
<.> Use the predicate we previously defined to search for substring in one column
<.> Use the repository methods implemented automatically by Micronaut Data to perform a search
<.> Map the `BookEntity` instances to the desired return type.
...
<.> Verify that addition of book info was successful by checking the status code.
callout:binding-json-array[]
callout:body-method[]
<.> Verify that there are exactly two books with `"Guide"` substring in title.
