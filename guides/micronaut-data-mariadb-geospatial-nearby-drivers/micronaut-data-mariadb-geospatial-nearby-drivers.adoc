...
The sample uses WGS 84 coordinates (`SRID 4326`), the coordinate system commonly used by GPS. `Srid.CrsType.GEOGRAPHIC` tells Micronaut Data to use the MariaDB spherical distance function for the 5 km spatial predicate, and the service calculates a WGS 84 geodetic distance in meters for each returned candidate.
...
Create a repository that declares the MariaDB dialect:

source:DeliveryDriverRepository[]

<1> `@JdbcRepository` tells Micronaut Data to generate a MariaDB JDBC repository.
