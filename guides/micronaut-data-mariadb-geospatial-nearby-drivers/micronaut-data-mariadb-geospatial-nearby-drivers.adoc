...
In this guide, you will build the dispatch part of a food delivery application. When a customer places an order, the application stores driver positions in MariaDB `GEOMETRY` columns and asks Micronaut Data JDBC for available drivers within 5 km of the order location. The dispatch service then chooses the closest returned candidate.

The sample uses WGS 84 coordinates (`SRID 4326`), the coordinate system commonly used by GPS. `Srid.CrsType.GEOGRAPHIC` tells Micronaut Data's MySQL dialect to use the spherical distance function for the 5 km spatial predicate.

common:requirements.adoc[]

* https://www.docker.io/gettingstarted/#h_installation[Docker] installed to run MariaDB with https://micronaut-projects.github.io/micronaut-test-resources/latest/guide/[Micronaut Test Resources].

common:completesolution.adoc[]

common:create-app-features.adoc[]

common:mariadb-driver.adoc[]
...
The MariaDB container image can take a while to download and initialize the first time. The container startup timeout and generated build's Test Resources client timeout give Micronaut Test Resources enough time to pull the image and start the container.

common:liquibase.adoc[]

resource:db/changelog/01-schema.xml[]

The migration creates the `delivery_driver` table with a MariaDB `GEOMETRY` column and a spatial index. In production, keep the spatial column and index statements in your migration tool instead of relying on test-only schema generation.
...
Micronaut Data geospatial model types use GeoJSON conversion by default. MariaDB stores the value in the `GEOMETRY` column declared in the Liquibase migration.
...
Create a repository that uses the MySQL dialect for MariaDB:

source:DeliveryDriverRepository[]

<1> Micronaut Data does not define a separate MariaDB dialect. Use `Dialect.MYSQL` for MariaDB JDBC repositories.
<2> The MySQL dialect translates the `Near` predicate to MariaDB-compatible `ST_Distance_Sphere(...) <= ?`.
...
When you run the test, Micronaut Test Resources starts a MariaDB container and configures the JDBC datasource automatically.
