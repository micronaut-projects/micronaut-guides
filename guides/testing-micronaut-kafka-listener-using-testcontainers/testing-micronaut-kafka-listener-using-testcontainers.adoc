...
We are going to create a Micronaut project with *Kafka*, *Micronaut Data* and *MySQL*,
...
The generated Java application has http://www.awaitility.org[Awaitility] library as test dependency
...
dependency:testcontainers-mysql[groupId=org.testcontainers,scope=test]
...
resource:application.toml[app=testcontainers,tag=datasource]
:exclude-for-languages:
...
First let us start with creating a `Product` persistence entity.

source:Product[app=testcontainers]

== Create Micronaut Data repository

Let us create a Micronaut Data repository for the *Product* entity and
...
<.> Micronaut Data implements the derived `updateByCode` method at compile time.
:exclude-for-languages:
...
:exclude-for-languages:
:exclude-for-languages:java
<.> Supply the driver class name configuration to the test.
<.> Configure the https://java.testcontainers.org/modules/databases/jdbc/[Testcontainers special JDBC URL] to spin up a MySQL container and configure it as a DataSource with Micronaut application context.
<.> Start a Kafka container for the test.
:exclude-for-languages:
...
:exclude-for-languages:
:exclude-for-languages:java
<.> As Kafka message processing is an asynchronous process, the test polls until the product price is updated in the database. If the message is consumed and processed within 10 seconds the test will pass, otherwise the test will fail.
:exclude-for-languages:
