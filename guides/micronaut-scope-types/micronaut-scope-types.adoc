...
The following route function injects two collaborators.
...
:exclude-for-languages:java,groovy,kotlin
<.> Inject `RobotFather` into an annotated module variable.
<.> Inject `RobotMother` into an annotated module variable.
<.> The @api@/io/micronaut/http/annotation/Get.html[@Get] annotation maps the route function to `/singleton`.
:exclude-for-languages:
...
:exclude-for-languages:
:exclude-for-languages:java,groovy,kotlin
<.> A new `Robot` instance is created for each HTTP request.
:exclude-for-languages:
...
<.> Use `requests.with_context(my_context)` to call the application under test with a context-bound HTTP client.
:exclude-for-languages:
<.> Both injection points, `RobotFather` and `RobotMother`, are fulfilled with the same instance of `Robot`. An instance associated with the HTTP Request.
<.> Both injection points are fulfilled with the new instance of `Robot`.
...
<.> Hitting the refresh endpoint, publishes a https://docs.micronaut.io/latest/api/io/micronaut/runtime/context/scope/refresh/RefreshEvent.html[RefreshEvent], which invalidates the instance of `Robot`.
...
source:context/FrameworkConfiguration[]
