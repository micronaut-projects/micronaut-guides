...
mn @cli-command@ \
   ...
   --features=data-jdbc,jakarta-persistence-api,postgres,liquibase \
   --build=@build@ \
   --lang=@lang@ \
----
...
NOTE: If you use https://launch.micronaut.io[Micronaut Launch], select "Micronaut Application" as application type and add `postgres`, `data-jdbc`, `jakarta-persistence-api`, and `liquibase` as features.
