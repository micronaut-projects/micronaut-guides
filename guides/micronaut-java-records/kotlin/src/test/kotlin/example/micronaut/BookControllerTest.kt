...
class BookControllerTest(
    ...
    private val booksInsertedInDbByMigration = 1

    @Test
    fun dataClassesUsedForJsonSerialization() {
        val title = "Building Microservices"
        val isbn = "1491950358"
        // <4>
        val about = """
        ...
        val books = client.retrieve(
            ...
            Argument.listOf(BookForSale::class.java)
        ) // <5>
        assertNotNull(books)
        assertEquals(booksInsertedInDbByMigration + 1, books.size)
        assertEquals("Building Microservices", books[1].title)
        assertEquals("1491950358", books[1].isbn)
        assertEquals(BigDecimal("46.16"), books[1].price)
