...
You will then connect the application to https://azure.microsoft.com/en-us/services/mysql/[Azure Database for MySQL].
...
1. First, you'll create a resource group that contains the database. It is recommended that you create a new resource group just for this guide rather than reusing an existing one.
...
* `AZURE_DATABASE_NAME` - The name of the database that will be created in the MySQL instance. You can change this value if you prefer another name.
...
The command should result in a response like:
...
NOTE: The command uses the `--version` flag to create a MySQL 8 instance; see the Azure documentation for https://docs.microsoft.com/en-us/azure/mysql/concepts-supported-versions[supported MySQL versions].
...
Learn more about https://docs.microsoft.com/en-us/azure/mysql/flexible-server/[Azure Database for MySQL flexible server].
