...
:exclude-for-languages:java,groovy,kotlin
resource:application.toml[tag=db]
:exclude-for-languages:
...
dependency:testcontainers-mongodb[groupId=org.testcontainers,scope=test]
dependency:testcontainers[groupId=org.testcontainers,scope=test]
:dependencies:

<<test-resources,Test Resources>> will provide us with a MongoDB instance for local testing and execution.

:exclude-for-languages:

:exclude-for-languages:java,groovy,kotlin

The Python test uses `requests` with the Micronaut test context to exercise the application's API.
