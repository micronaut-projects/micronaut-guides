...
<.> This method simply returns a model map that will be rendered as JSON since there's no `@View` annotation
...
We used placeholders in `application.properties` for the `client-id`, `client-secret`, and the `issuer` URL properties, so we need to create `OAUTH_CLIENT_ID`, `OAUTH_CLIENT_SECRET`, and `OAUTH_ISSUER` environment variables.
