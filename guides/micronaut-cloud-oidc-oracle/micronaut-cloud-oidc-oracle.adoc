...
<.> Set `micronaut.security.authentication` as `idtoken`. The `idtoken` provided by your OAuth 2.0 application when the Authorization code flow ends (a signed JWT) will be saved in a cookie.
callout:oauth2-client-name[arg0=micronaut]
<.> `client-id`, `client-secret`, and the `issuer` URL will be set as environment variables.
...
:exclude-for-languages:
:exclude-for-languages:java,groovy,kotlin
Create route functions to test the security configuration:
:exclude-for-languages:
...
<.> This method returns a model map that will be rendered as JSON because there is no `@View` annotation.
...
:exclude-for-languages:
:exclude-for-languages:java,groovy,kotlin
You used placeholders in `application.toml` for the `client-id`, `client-secret`, and the `issuer` URL properties, so create `OAUTH_CLIENT_ID`, `OAUTH_CLIENT_SECRET`, and `OAUTH_ISSUER` environment variables.
:exclude-for-languages:
