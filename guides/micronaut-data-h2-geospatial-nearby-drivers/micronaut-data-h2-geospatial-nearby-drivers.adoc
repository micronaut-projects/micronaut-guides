...
In this guide, you will build the dispatch part of a food delivery application. When a customer places an order, the application stores driver positions in H2 `GEOMETRY` columns and asks Micronaut Data JDBC for available drivers near the order location. The dispatch service then chooses the closest candidate within 5 km.

The sample uses WGS 84 coordinates (`SRID 4326`), the coordinate system commonly used by GPS. H2GIS evaluates the spatial predicate with the geometry coordinate system.
...
<2> The `Near` predicate is translated to H2GIS `ST_DWithin`.
