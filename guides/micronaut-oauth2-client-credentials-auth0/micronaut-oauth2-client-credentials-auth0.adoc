...
<7> Propagate the access token obtained from Auth0 to requests sent to the services `bookinventory` and `bookcatalogue`. It uses the https://micronaut-projects.github.io/micronaut-security/latest/api/index.html[Micronaut Client Credentials HTTP Client Filter].
...
Create an interface to fetch the configuration value: 
...
To include the `audience` key in the request, replace the bean of type `ClientCredentialsClient` qualified by a Name qualifier with value `auth0`. Use a common pattern in Micronaut Applications. I like to call it Extend, Replace, Override. You extend from a class the framework ships with, you annotate it with `@Replaces` and you override a method to fulfil your application needs.

source:Auth0ClientCredentials[app=bookrecommendation]

callout:at-named[number=1]
<2> https://docs.micronaut.io/latest/guide/#replaces[Replace] the bean of type `ClientCredentialsClient`
