...
Update the build for all three applications (`bookcatalogue`, `bookinventory`, and `bookrecommendation`); add https://micronaut-projects.github.io/micronaut-security/latest/guide/#jwt[Micronaut JWT] and https://micronaut-projects.github.io/micronaut-security/latest/guide/#oauth[Micronaut OAuth 2.0] dependencies:
...
resource:application.properties[app=bookinventory,tag=oauth2]
:exclude-for-languages:

:exclude-for-languages:java,groovy,kotlin
resource:application.toml[app=bookinventory,tag=oauth2]
:exclude-for-languages:
...
:exclude-for-languages:python
resource:application.properties[app=bookcatalogue,tag=oauth2]
:exclude-for-languages:

:exclude-for-languages:java,groovy,kotlin
resource:application.toml[app=bookcatalogue,tag=oauth2]
:exclude-for-languages:
...
resource:application-dev.properties[app=bookrecommendation,tag=httpservices]
:exclude-for-languages:

:exclude-for-languages:java,groovy,kotlin
Configure the declarative HTTP clients to point to the other microservice URLs.

resource:application.toml[app=bookrecommendation,tag=httpservices]
:exclude-for-languages:
...
:exclude-for-languages:python
resource:application.properties[app=bookrecommendation,tag=oauth2]
:exclude-for-languages:

:exclude-for-languages:java,groovy,kotlin
resource:application.toml[app=bookrecommendation,tag=oauth2]
:exclude-for-languages:
...
The previous configuration uses several placeholders with default values. You will need to set up `OAUTH_CLIENT_ID`, `OAUTH_CLIENT_SECRET`, `OAUTH_TOKEN_URL`, and `OAUTH_SCOPE` environment variables in your Oracle Cloud Identity Domain application.
...
export OAUTH_SCOPE=BookInventory
----
...
To run the application, execute `pyronaut run`.
