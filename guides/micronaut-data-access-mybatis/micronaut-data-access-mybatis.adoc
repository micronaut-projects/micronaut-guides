...
<1> Add the MyBatis dependency.
<2> Configure SQL `DataSource` instances using Hikari Connection Pool.
<3> Add a dependency to the in-memory H2 database.
...
As there is no out-of-the-box support yet in Micronaut Framework for MyBatis, it is necessary to manually wire `SqlSessionFactory`.

Create the following https://docs.micronaut.io/latest/guide/#factories[@Factory] class:

source:MybatisFactory[]
<.> Annotate the class with `@Factory`.
callout:constructor-di[arg0=DataSource]
<.> Define a `@Bean` of type `SqlSessionFactory`.
<.> Use the `dataSource` to create a new MyBatis environment.
<.> Define the package to scan for mappers.
<.> Create a new `SqlSessionFactory` bean.
...
callout:singleton[]
<.> Easily inject the `SqlSessionFactory` bean created by the `@Factory`.
<.> Use _try-with-resources_ to automatically close the SQL session.
<.> Get MyBatis mapper implementation for the interface.
<.> Execute the desired method using the mapper. This will trigger the SQL query.
<.> In a database write access, commit the transaction.
...
<.> Maps a `GET` request to `/genres/{id}`, which attempts to show a genre. This illustrates the use of a URL path variable.
<.> Returning `null` when the genre doesn't exist makes Micronaut Framework respond with 404 (not found).
<.> Maps a `PUT` request to `/genres`, which attempts to update a genre.
<.> Add `@Valid` to any method parameter that requires validation. Use a POJO supplied as a JSON payload in the request to populate the command.
<.> It is easy to add custom headers to the response.
<.> Maps a `GET` request to `/genres`, which returns a list of genres. This mapping illustrates optional URL parameters.
<.> Maps a `POST` request to `/genres`, which attempts to save a genre.
<.> Maps a `DELETE` request to `/genres/{id}`, which attempts to remove a genre. This illustrates the use of a URL path variable.
...
<.> If you care only about the object in the response, use `retrieve`.
...
Read more about the https://docs.micronaut.io/latest/guide/#dataAccess[Configurations for Data Access] section and
