...
usage() {
  echo "$0: Time to first request for native or docker applications"
  echo ""
  echo "  $0 [-d|-n] [-p port] ARTIFACT"
  echo ""
...
if [ $# -eq 0 ]; then
  echo "Needs the docker image or native executable to run"
  exit 1
fi
