...
If you use https://launch.micronaut.io[Micronaut Launch], select serverless function as the application type and add the `aws-lambda` feature.
...
Create a handler that uses the Micronaut framework's dependency injection engine to inject its collaborators.
...
<6> Find the current secret primary key.
<7> Generate the new secret content, a JSON object serialized as a string.
