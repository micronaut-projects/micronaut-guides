...
The generated Pyronaut application configures logging in `src/main.py` and logs at INFO by default.
...
callout:singleton[]
<.> Listen to the event `ServerStartupEvent`
<.> Constructor injection of `RegisterUseCase`
callout:application-event-listener-startup-event[]
