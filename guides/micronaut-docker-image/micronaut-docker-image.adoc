...
* `optimizedDockerBuild` - Builds an optimized application and packages it into a Docker image.
* `optimizedDockerBuildNative` - Builds a native image containing the optimized application and packages it into a Docker image.
...
== Building a Docker Image with Pyronaut

The `pyronaut build` command can build Docker images for Pyronaut applications:

* `pyronaut build --docker` - Builds a JVM Docker image
* `pyronaut build --native --docker` - Builds a native Docker image using GraalVM Native Image inside Docker
...
----
...
Docker image build complete: default:1.0.0
----

:exclude-for-languages:

Supply the image name and tag to the time to first request script:
...
----
...
Docker image build complete: default:1.0.0-native
----
...
[source, bash]
----
./ttfr.sh default:1.0.0-native
default:1.0.0-native
...
.
0.047
seconds
...
Run the time to first request script with the CRaC Docker image:
...
Run the time to first request script with the AOT optimized Docker image:
...
Micronaut Framework offers many options for packaging your application as a Docker image. As illustrated in the following chart, you can speed up your Docker images by using the https://docs.micronaut.io/latest/guide/#graal[GraalVM integration in Micronaut Framework], the https://micronaut-projects.github.io/micronaut-crac/latest/guide/[CRaC (Coordinated Restore at checkpoint) integration in Micronaut Framework], the https://micronaut-projects.github.io/micronaut-gradle-plugin/latest/#_micronaut_aot_plugin[Micronaut AOT Gradle Plugin], or https://micronaut-projects.github.io/micronaut-maven-plugin/latest/examples/aot.html[Maven Plugin integration with Micronaut AOT].
...
Pyronaut supports JVM Docker images and native Docker images. Native Docker images compile inside a container, which keeps the image Linux-compatible even when the build starts from macOS.
