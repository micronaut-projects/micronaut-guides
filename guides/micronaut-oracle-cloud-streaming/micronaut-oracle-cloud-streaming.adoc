...
<1> Annotate with `@Introspected` to generate metadata at compilation time and avoid reflection
...
<1> Use the `@Controller` annotation to make the class a controller, and specify the root URI as `"/game"`
<2> Execute controller actions on the I/O executor to avoid blocking the Event loop
