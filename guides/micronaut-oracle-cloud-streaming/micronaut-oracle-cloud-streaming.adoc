...
The microservices will use Oracle Cloud Streaming and the Kafka API to send and receive messages. The messages will represent events occurring chess game play, including game start and end, and each move.
...
Create the `chess-game` microservice using the https://docs.micronaut.io/latest/guide/index.html#cli[Micronaut Command Line Interface] or with https://launch.micronaut.io[Micronaut Launch].

[source,bash]
----
mn create-app --features=kafka,graalvm,testcontainers example.micronaut.chess-game --build=@build@ --lang=@lang@
----
...
<1> Annotate the class with `@KafkaClient` to declare it as a Kafka message producer; Micronaut will generate the sending code at compile time
<2> Annotate the method with `@Topic` and specify the topic name
<3> By returning a reactive type, Micronaut will use non-blocking code to send the message
...
<1> Execute controller actions on the I/O executor to avoid blocking the Event loop
<2> The method accepts HTTP POST requests to indicate a new game has started
<3> Returning a reactive type to configure a reactive response
<4> Assign the game's primary key as a UUID
<5> Use the `GameReporter` to send a Kafka message with new game data
<6> The method accepts HTTP POST requests to indicate a move has occurred
<7> Use the `GameReporter` to send a Kafka message with game move data
<8> The method accepts HTTP POST requests to indicate the game ended with a checkmate
<9> Use the `GameReporter` to send a Kafka message that the game ended in a checkmate
<10> The method accepts HTTP POST requests to indicate the game ended with a draw
<11> Use the `GameReporter` to send a Kafka message that the game ended in a draw
...
<1> Configure Micronaut to look for static resources in `src/main/resources/public`
...
Create the `chess-listener` microservice using the https://docs.micronaut.io/latest/guide/index.html#cli[Micronaut Command Line Interface] or with https://launch.micronaut.io[Micronaut Launch].

[source,bash]
----
mn create-app --features=kafka,graalvm,data-jdbc,flyway,testcontainers example.micronaut.chess-listener --build=@build@ --lang=@lang@
----

include::{commondir}/common-build-lang-arguments.adoc[]

If you use Micronaut Launch, select Micronaut Application as application type and add the `kafka`, `graalvm`, `data-jdbc`, `flyway`, and `testcontainers` features.
...
The same `GameDTO` and `GameStateDTO` data transfer objects as above in the `chess-game` microservice. In a real application these would be in a shared library but to keep things simple we'll just duplicate them.
...
<1> Annotate with `@JdbcRepository` to make this a Micronaut Data JDBC repository, and specify the `H2` dialect. Micronaut will generate persistence logic at compile time and use H2-specific SQL.
<2> Restrict the bean to only be available in the development environment
...
<1> Annotate the class with `@KafkaListener` to declare it as a Kafka message listener; Micronaut will generate the receiving code at compile time
...
Add the required dependencies to the `chess-listener` microservice to support using ATP:
...
Create `src/main/resources/application-oraclecloud.yml`. Micronaut applies this configuration file only for the `oraclecloud` environment.
...
<1> Annotate with `@JdbcRepository` to make this a Micronaut Data JDBC repository, and specify the `ORACLE` dialect. Micronaut will generate persistence logic at compile time and use Oracle-specific SQL.
<2> Restrict the bean to only be available in the `oraclecloud` environment
...
Select the compartment where you created the streams from the drop-down and click "Create Policy":
...
<6> Dependency injection for an HTTP client that Micronaut will implement at compile to make calls to `GameController`
...
<11> Use the `HttpClient` to send a POST request that indicates a game move occurred, which will trigger sending a message with Kafka
...
To use this we need to update the config, and create a dynamic group and add policy statements granting permissions.
...
include::{commondir}/common-graal-with-plugins-multi.adoc[]
...
Also see https://guides.micronaut.io/latest/micronaut-kafka.html[this guide on Micronaut + Kafka].
