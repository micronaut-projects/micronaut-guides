...
[source,bash]
----
mn create-app --features=kafka,graalvm example.micronaut.chess-game --build=@build@ --lang=@lang@
----
...
[source,bash]
----
mn create-app --features=kafka,graalvm,data-jdbc,flyway example.micronaut.chess-listener --build=@build@ --lang=@lang@
----

include::{commondir}/common-build-lang-arguments.adoc[]

If you use Micronaut Launch, select Micronaut Application as application type and add the `kafka`, `graalvm`, `data-jdbc`, and `flyway` features.
...
and a `H2GameStateRepository` interface that extends `GameStateRepository`:
