...
:dependencies:
dependency:testcontainers-kafka[groupId=org.testcontainers,scope=test]
...
callout:executes-on[]
<.> The method accepts HTTP POST requests to indicate a new game has started
<.> The method accepts encoded form data (the two players' names)
<.> The response will be plain text and contain the ID of the persisted `Game`
<.> Send a `"Created"` (201) status to indicate a `Game` was persisted
<.> Return a reactive type to configure a reactive response
<.> Assign the game's primary key as a UUID
<.> Use the `GameReporter` to send a Kafka message with new game data
<.> The method accepts HTTP POST requests to indicate a move has occurred
<.> The method accepts encoded form data (the move information)
<.> Send a `"Created"` (201) status to indicate a `GameState` was persisted
<.> Use the `GameReporter` to send a Kafka message with game move data
<.> The method accepts HTTP POST requests to indicate the game ended with a checkmate
<.> Send a `"No Content"` (204) status to indicate success and no response body
<.> Use the `GameReporter` to send a Kafka message that the game ended in a checkmate
<.> The method accepts HTTP POST requests to indicate the game ended with a draw
<.> Send a `"No Content"` (204) status to indicate success and no response body
<.> Use the `GameReporter` to send a Kafka message that the game ended in a draw
...
:exclude-for-languages:python
resource:application-dev.properties[app=chess-game]
:exclude-for-languages:
:exclude-for-languages:java,groovy,kotlin
resource:application-dev.toml[app=chess-game]
:exclude-for-languages:
...
:exclude-for-languages:
:exclude-for-languages:java,groovy,kotlin
resource:application.toml[app=chess-game,tag=static-resources]
:exclude-for-languages:
...
:exclude-for-languages:
:exclude-for-languages:java,groovy,kotlin
resource:application.toml[app=chess-listener,tag=flyway]
:exclude-for-languages:
...
Create a `GameRepository` interface to have access to methods for `Game` entity persistence:

source:chess/repository/GameRepository[app=chess-listener]

:exclude-for-languages:java,groovy,kotlin
<1> Annotate with `@JdbcRepository` to make this a Micronaut Data JDBC repository.
<2> Restrict the bean to be available in the `oraclecloud` and test environments.
:exclude-for-languages:
...
Create a `GameStateRepository` interface to have access to methods for `GameState` entity persistence:
...
:exclude-for-languages:python
resource:application-dev.properties[app=chess-listener]
:exclude-for-languages:
:exclude-for-languages:java,groovy,kotlin
resource:application-dev.toml[app=chess-listener]
:exclude-for-languages:
...
:exclude-for-languages:python
resource:application-oraclecloud.properties[app=chess-listener,tag=atp]
:exclude-for-languages:
:exclude-for-languages:java,groovy,kotlin
resource:application-oraclecloud.toml[app=chess-listener,tag=atp]
:exclude-for-languages:
...
:exclude-for-languages:python
resource:application-oraclecloud.properties[app=chess-game]
:exclude-for-languages:
:exclude-for-languages:java,groovy,kotlin
resource:application-oraclecloud.toml[app=chess-game,tag=kafka]
:exclude-for-languages:
...
<.> Use the `@Testcontainers` annotation to configure automatic container management (not necessary in Spock tests)
callout:test-instance-per-class[]
<.> Implementing `TestPropertyProvider` allows the test class to provide application configuration properties, in this case the dynamically allocated Kafka broker port
<.> The Testcontainers instance for Kafka
<.> Dependency injection for the `ChessListener` class declared below, a Kafka listener class that replicates the functionality of the class of the same name in the `chess-listener` microservice
<.> Dependency injection for an HTTP client that the Micronaut framework will implement at compile time to make calls to `GameController`
:exclude-for-languages:java,groovy
callout:atfield[]
callout:lateinit[]
<.> Wait a few seconds for the message to arrive; it should happen very quickly, but the message will be sent on a separate thread
<.> Configure the Kafka broker port (it will be a different unused port each time) so Micronaut Kafka clients and listeners connect to the test broker
<.> Use a Map to hold form parameter names and values
<.> Use the `HttpClient` to send a POST request that indicates a game has started, which will trigger sending a message with Kafka
<.> Use the `HttpClient` to send a POST request that indicates a game move has occurred, which will trigger sending a message with Kafka
<.> Use the `HttpClient` to send a POST request that indicates a game has ended (in either a draw or checkmate), which will trigger sending a message with Kafka
:exclude-for-languages:
:exclude-for-languages:kotlin
<.> Wait a few seconds for the message to arrive; it should happen very quickly, but the message will be sent on a separate thread
<.> Configure the Kafka broker port (it will be a different unused port each time) so Micronaut Kafka clients and listeners connect to the test broker
...
<.> Implementing `TestPropertyProvider` allows the test class to provide application configuration properties, in this case the dynamically allocated Kafka broker port
<.> The Testcontainers instance for Kafka
<.> Dependency injection for the `GameReporter` interface declared below, a Kafka producer interface that replicates the functionality of the class of the same name in the `chess-game` microservice
<.> Wait a few seconds for the message to arrive; it should happen very quickly, but the message will be sent on a separate thread
<.> Configure the Kafka broker port (it will be a different unused port each time) so Micronaut Kafka clients and listeners connect to the test broker
...
:exclude-for-languages:python
testResource:application-test.properties[app=chess-listener]
:exclude-for-languages:
:exclude-for-languages:java,groovy,kotlin
testResource:application-test.toml[app=chess-listener]
:exclude-for-languages:
