...
Replace the client creation in the tutorial with the steps illustrated in the following screenshots.
...
:exclude-for-languages:python
resource:application-dev.properties[tag=port]
:exclude-for-languages:
:exclude-for-languages:java
resource:application.toml[tag=port]
:exclude-for-languages:
...
To use the https://github.com/HubSpot/jinjava[Jinjava] template engine to render views in a Micronaut application, add the following dependency to your classpath.
...
:exclude-for-languages:python
resource:application-dev.properties[tag=oauth2]
:exclude-for-languages:
:exclude-for-languages:java
...
<6> Accept GET requests to the `/logout` endpoint.
...
:exclude-for-languages:python
Create a controller to handle the requests to `/`. You will display the email of the authenticated person if any. Annotate the controller endpoint with `@View` since we will use a Jinjava template.
:exclude-for-languages:
:exclude-for-languages:java
Create a route function to handle the requests to `/`. You will display the email of the authenticated person if any. Annotate the route with `@View` since we will use a Jinjava template.
:exclude-for-languages:
...
Create a Jinjava template:

resource:views/home.html[]

Also, note that we return an empty model in the controller. However, we are accessing `security` in the Jinjava template.
...
Create an https://micronaut-projects.github.io/micronaut-security/latest/api/io/micronaut/security/oauth2/endpoint/endsession/request/EndSessionEndpointResolver.html[EndSessionEndpointResolver] replacement to log out from Keycloak.
...
To run the application, use the `pyronaut run` command, which starts the application on port 8081.
...
Check out the https://www.keycloak.org[Keycloak] website.
