...
class ValidateSecondarySignedJwtTest {
    ...
    fun canValidateAJWebTokenSignedWithTheSecondaryJsonWebKey() {
        ...
        val signatureGeneratorConfiguration: SignatureGeneratorConfiguration = RSASignatureGenerator(MockSignatureGenerator(jwkConfiguration))
        val tokenGenerator: TokenGenerator = JwtTokenGenerator(signatureGeneratorConfiguration, null, claimsGenerator)
        val jwtOptional = tokenGenerator.generateToken(mapOf("sub" to "sergio"))
        assertTrue(jwtOptional.isPresent)

        val jwtString = jwtOptional.get()
        assertDoesNotThrow<JWT> { JWTParser.parse(jwtString) }
    ...
    internal class MockSignatureGenerator(jwkConfiguration: JwkConfiguration) :
            AbstractRSAGeneratorSignatureConfiguration(jwkConfiguration.secondary)
}
