...
In this guide, you load a primary and secondary JWK from their JSON representation. Both keys participate as token validators. Moreover, the primary participates in token generatation. Also, you expose their public keys in as JWKS in a `/keys` endpoint.
...
<1> `@Refreshable` scope is a custom scope that allows a bean’s state to be refreshed via the https://docs.micronaut.io/latest/guide/index.html#refreshEndpoint[/refresh] endpoint.

=== Primary Key

Create an abstract class which extends from `AbstractRSASignatureConfiguration` but also implements `AbstractRSASignatureConfiguration`.
...
Create a test which verifies that the application can generate a signed JSON Web Token.

test:TokenGeneratorTest[]

=== Secondary signature validation

Create a test which verifies that the application can validate a JSON Web Token signed with the secondary key.

test:ValidateSecondarySignedJwtTest[]

=== Keys test

Create a test which verifies that the application exposes a keys endpoint:
...
Read more about https://micronaut-projects.github.io/micronaut-security/latest/guide/index.html[Micronaut Security].

Checkout the tutorial https://guides.micronaut.io/latest/micronaut-cli-jwkgen.html[JWK generation with Micronaut Command Line Application].
