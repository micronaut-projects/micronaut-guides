...
<.> Use the username and password to create the `Session`.
...
callout:text-plain[]
<.> You can send plain-text emails.
<.> You can send HTML emails leveraging Micronaut template rendering capabilities.
callout:consumes[]
<.> You can send email with attachments.
...
resource:application.properties[tag=micronaut-email]
:exclude-for-languages:
:exclude-for-languages:java,groovy,kotlin
If you always use the same sender, you can add the following configuration snippet to `application.toml`:


resource:application.toml[tag=micronaut-email]
:exclude-for-languages:
...
:exclude-for-languages:
:exclude-for-languages:java,groovy,kotlin
Add the following snippet to `application.toml` to supply the SMTP credentials.
:exclude-for-languages:
...
:exclude-for-languages:python
resource:application.properties[tag=smtp]
:exclude-for-languages:
:exclude-for-languages:java,groovy,kotlin
resource:application.toml[tag=smtp]
:exclude-for-languages:
...
resource:application.properties[tag=javamail]
:exclude-for-languages:
:exclude-for-languages:java,groovy,kotlin
Add the following snippet to `application.toml` to supply JavaMail properties:

resource:application.toml[tag=javamail]
:exclude-for-languages:
