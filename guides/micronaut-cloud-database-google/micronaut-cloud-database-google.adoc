...
You will create the database with the Google Cloud CLI. See the https://cloud.google.com/sdk/gcloud/reference/sql[MySQL CLI command reference] for more information.
...
* `--database-version` - Specifies MySQL v8.0. See https://cloud.google.com/sql/docs/mysql/admin-api/rest/v1beta4/SqlDatabaseVersion[SQL DB versions] for other options.
...
* `--authorized-networks` - A single IP address or a range of IP addresses in CIDR notation.
* `LOCAL_IP_ADDRESS` - This is the local IP address where the Micronaut application is running. You can find the value for your local machine by visiting http://whatismyip.akamai.com/[http://whatismyip.akamai.com/].
...
Make note of the `"Primary Address"`; you will use it later.
...
Instead of using the _root_ database user account, create one for the application to use:
...
Now that the MySQL instance is running, create a database.
...
Create environment variables for `DATASOURCES_DEFAULT_URL`, `DATASOURCES_DEFAULT_USERNAME`, and `DATASOURCES_DEFAULT_PASSWORD`, which will be used in the Micronaut app's `application.properties` data source:
...
----
* `DB_INSTANCE_IP` - Your MySQL instance's primary address.
