...
The previous command creates a folder named `bookcatalogue, and a Micronaut app inside it with default package:
`example.micronaut`.
...
The previous command creates a folder named `bookinventory`, and a Micronaut app inside it with default package :
`example.micronaut`.
...
default package: `example.micronaut`.

Add `rabbitmq` dependency. In this microservice we will use Micronaut HTTP Server to receive REST request, so it is
not necessary to remove any dependency.
