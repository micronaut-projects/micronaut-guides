...
One option is create them directly in the RabbitMQ Admin UI available on `http://localhost:15672[http://localhost:15672^]`.
Use `guest` for both username and password.

Another option is create them programmatically. Create the class `ChannelPoolListener.java`:
...
As we did in the `Catalogue` Microservice, create the class `ChannelPoolListener.java` in `bookinventory/src/main/java/example/micronaut/bookcatalogue/ChannelPoolListener.java`
...
As we did in the `Catalogue` Microservice, create the class `ChannelPoolListener.java` in `bookrecommendation/src/main/java/example/micronaut/bookcatalogue/ChannelPoolListener.java`
with the same content as before.
