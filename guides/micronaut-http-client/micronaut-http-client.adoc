...
<1> callout:controller[arg0=/github]
...
<4> callout:singleresult[]
<5> callout:get[arg0=fetchReleases,arg1=/releases]
...
<1> callout:spec-name[]
<2> callout:mock-http-server[arg0=GitHub]
...
<5> callout:binding-json-array[]
<6> Use `status` to check the HTTP status code.
<7> callout:body-method[]
...
<3> Kotlin doesn't support runtime repeatable annotations (see https://youtrack.jetbrains.com/issue/KT-12794[KT-12794]. We use a custom condition to enable the bean where appropriate.
:exclude-for-languages:

<4> Constructor injection of the configuration parameters.
