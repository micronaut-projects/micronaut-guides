...
<1> The `@MappedEntity` annotation maps the class to the collection.
<2> Use `@NotBlank` to ensure the `name` is not blank.
<3> `@Id` and `@GeneratedValue` define the generated identifier.
:exclude-for-languages:
...
:exclude-for-languages:
:exclude-for-languages:java,groovy,kotlin
<1> Inject the `FruitService` into an annotated module variable.
<2> Execute blocking MongoDB operations on the blocking executor.
<3> Maps a `GET` request to `/fruits`.
<4> Maps a `POST` request to `/fruits`.
<5> You can specify the HTTP status code via the `@Status` annotation.
<6> Add `@Valid` to any method parameter that requires validation.
<7> Maps a `GET` request to `/fruits/{id}`.
<8> Maps a `GET` request to `/fruits/q`.
...
:exclude-for-languages:
:exclude-for-languages:java,groovy,kotlin
Create pytest tests that use the Pyronaut `requests` integration to call the application's API.
:exclude-for-languages:
