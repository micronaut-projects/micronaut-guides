...
* A PostgreSQL instance provided by <<test-resources,Test Resources>> and running in Docker.
...
<1> Authors will be stored in a table separate from the ToDos. They just have a username.
...
<4> When GraphQL loads a ToDo, it uses this to fetch the authors if they are required in the response.
...
For testing the application use a context-bound Pyronaut requests client to send a `POST` request to the `/graphql` endpoint.
Create the following test module:
...
resource:application.toml[tag=graphiql]
