...
<3> `startApplication<Application>` here does the work of `.build().mainClass(Application::class.java).start()`. Convenient!
...
Note that while *we* are creating this client for the purposes of this guide, but often times something like this may come from
