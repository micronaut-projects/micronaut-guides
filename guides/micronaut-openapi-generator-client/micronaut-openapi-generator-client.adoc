...
instead.
<5> Specify that this authorization should be used for all client requests (You would probably want to modify this value
if you wanted to use multiple clients or reach paths that do not require authorization).
...
automatically converting instances to JSON in response.
...
<4> Specify that the method requires two parameters: a query to search the tweets by, and the number of most frequent words to
return.
