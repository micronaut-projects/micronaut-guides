...
As of Micronaut Security version 3.2, applications can use https://en.wikipedia.org/wiki/X.509[X.509] client certificates when using SSL to enable https://en.wikipedia.org/wiki/Mutual_authentication[mutual authentication], i.e. in addition to the client verifying the server identity with its SSL certificate, the server can verify the client identity with a client certificate.
...
Run the native image and repeat the cURL calls and browser request that you made earlier.
