...
Replace the generated `application.toml` with this:
...
<.> Allow authenticated and non-authenticated requests
callout:text-plain[]
:exclude-for-languages:python
<.> Bind the `X509Authentication` if present, but allow null when not authenticated
<.> Redundantly bind the `Authentication` to verify that it's the same instance as the `X509Authentication`
:exclude-for-languages:
:exclude-for-languages:java,groovy,kotlin
<.> Bind the `X509Authentication` and `Authentication` if present, but default both to `None` when not authenticated
:exclude-for-languages:
<.> Default the username for non-authenticated requests
<.> Sanity check that there's no `Authentication` if there's no `X509Authentication`
<.> Sanity check that the `Authentication` is the `X509Authentication`
<.> Build the response as the username plus info about the certificate CA for the authenticated user
...
testResource:application-test.properties[tag=client]

:exclude-for-languages:

:exclude-for-languages:java,groovy,kotlin

Create `application-test.toml` in `tests-config` with this content:

testResource:application-test.toml[tag=client]
