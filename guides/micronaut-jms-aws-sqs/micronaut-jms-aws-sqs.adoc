...
<3> Maps a `POST` request to the `/demo` path, which attempts to publish a message to an SQS queue instance.
...
<1> Create a Micronaut test fixture that initializes the application context and embedded server.
...
<2> Start a Floci container before creating the Micronaut test context.
<3> Provide the SQS connection properties from the running Floci container.
