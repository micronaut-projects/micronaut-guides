...
Create a `@MappedEntity` to save persons. Initially, consider name and age required. Use the `int` primitive for the age.
...
The tables `databasechangelog`, `databasechangeloglock` are used by Liquibase to keep track of database migrations.
...
Create a test which invokes the Liquibase endpoint
