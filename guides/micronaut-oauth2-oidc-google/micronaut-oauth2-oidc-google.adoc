...
Also add https://docs.micronaut.io/latest/guide/#jwt[Micronaut JWT support^] dependencies:
...
- The @api@/io/micronaut/views/model/security/SecurityViewModelProcessor.html[SecurityViewModelProcessor^] injects into the model a `security` map with the authenticated user. See https://micronaut-projects.github.io/micronaut-views/latest/guide/#views-security[User in a view^] documentation.
