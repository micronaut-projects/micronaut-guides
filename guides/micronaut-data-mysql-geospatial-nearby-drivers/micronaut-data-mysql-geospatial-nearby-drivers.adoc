...
In this guide, you will build the dispatch part of a food delivery application. When a customer places an order, the application stores driver positions in MySQL `GEOMETRY` columns and asks Micronaut Data JDBC for available drivers near the order location. The dispatch service then chooses the closest candidate within 5 km.

The sample uses WGS 84 coordinates (`SRID 4326`), the coordinate system commonly used by GPS. With MySQL 8, `ST_Distance` returns meters for geographic coordinates in `SRID 4326`, so the repository enforces the 5 km dispatch rule.

common:requirements.adoc[]

* https://www.docker.io/gettingstarted/#h_installation[Docker] installed to run MySQL with https://micronaut-projects.github.io/micronaut-test-resources/latest/guide/[Micronaut Test Resources].
...
The MySql image can take a while to download and initialize the first time. The container startup timeout and generated build's Test Resources client timeout give Micronaut Test Resources enough time to pull the image and start the container.

common:liquibase.adoc[]

resource:db/changelog/01-schema.xml[]

The migration creates the `delivery_driver` table with a MySQL `GEOMETRY` column and a spatial index. In production, keep the spatial column and index statements in your migration tool instead of relying on test-only schema generation.
...
Micronaut Data geospatial model types use GeoJSON conversion by default. MySQL stores the value in the `GEOMETRY` column declared in the Liquibase migration.
...
<2> The `Near` predicate is translated to MySQL `ST_Distance(...) <= ?`.
