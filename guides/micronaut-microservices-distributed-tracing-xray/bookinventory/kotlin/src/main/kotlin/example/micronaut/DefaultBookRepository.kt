...
open class DefaultBookRepository(
    ...
    override fun findByIsbn(@NotBlank isbn: String): Optional<Book> {
        val request = QueryRequest.builder()
            .tableName(dynamoConfiguration.tableName)
            .indexName(INDEX_GSI_2)
            .limit(1)
            .keyConditionExpression("#pk = :pk")
            .expressionAttributeNames(mapOf("#pk" to ATTRIBUTE_GSI_2_PK))
            .expressionAttributeValues(
                mapOf(":pk" to AttributeValue.builder().s(PREFIX_ISBN + HASH + isbn).build())
            )
            .build()
        ...
        return parseInResponse(response).firstOrNull()?.let { Optional.of(it) } ?: Optional.empty()
    }
