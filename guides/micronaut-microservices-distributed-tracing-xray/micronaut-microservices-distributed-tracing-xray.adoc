...
tracing information from a Micronaut application composed of three microservices and publish it to https://aws.amazon.com/xray/[AWS X-Ray].
...
resource:application.properties[app=bookrecommendation,tag=otel]
...
Each service disables OpenTelemetry for the test classpath:
...
The previous request generates a trace like this:
