...
:exclude-for-languages:
:exclude-for-languages:java,groovy,kotlin
<1> The route decorators map each function directly to a `/genres` URI.
<2> Inject beans into annotated module variables.
:exclude-for-languages:
<3> Maps a `GET` request to `/genres/{id}`, which attempts to show a genre. This illustrates the use of a URL path variable.
<4> Returning an empty Reactor publisher when the genre doesn't exist makes the Micronaut framework respond with 404 (not found).
<5> Maps a `PUT` request to `/genres`, which attempts to update a genre.
<6> Adds `@Valid` to any method parameter that requires validation. Use a POJO supplied as a JSON payload in the request to populate the command.
<7> The Reactor `thenReturn(..)` method is used to return a `NO_CONTENT` response only if the update was successful. Headers are easily added by the `HttpResponse` API.
...
<10> Maps a `POST` request to `/genres`, which attempts to save a genre. This example uses the Reactor `map(..)` method to obtain the result of the call to `save(String name)` and map it to a HTTP `CREATED` response if the save operation was successful.
<11> Maps a `POST` request to `/genres/ex`, which generates an exception. This example demonstrates how to handle exceptions with `onErrorReturn(..)`, which simply returns a `NO_CONTENT` response if an error occurs. More complex use cases (like logging the exception) can be handled with `onErrorResume(..)`.
<12> Maps a `DELETE` request to `/genres/{id}`, which attempts to remove a genre. This illustrates the use of a URL path variable. The Reactor `thenReturn(..)` method is used to return a `NO_CONTENT` response only if the call to `deleteById` is successful.
