...
In a different folder, create a static website. It makes an HTTP GET request with JavaScript and updates an HTML element with the response.

resource:../../../index.html[]

Add a controller that the static website consumes with JavaScript.
...
To use `jwebserver`, use Java 18 or greater in a terminal window. You can use https://sdkman.io[SDKMAN!] to get a list of available Java versions for your computer with `sdk list java`.
...
`jwebserver` serves the UI on port 8000.
...
resource:application.toml[tag=cors]
...
resource:application.toml[tags=cors|corsconfigurations]

:exclude-for-languages:

You can start the static server on a different port. For example, you could run `jwebserver -p 8001` and verify that it is not allowed. The application's configuration only allows requests with origin `127.0.0.1:8000`.
