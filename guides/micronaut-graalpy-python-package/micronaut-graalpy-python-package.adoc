...
=== Python code
We are going to use the https://www.pygal.org/en/stable/documentation/types/bar.html#stacked[StackedBar] class from the https://www.pygal.org/en/stable/[Pygal] package to render a graph
...
<.> Java method matching the `StackedBar.render()` Python function.
<.> Java interface with methods matching functions from the `Svg` Python class we want to use.
<.> Java method matching the `Svg.decode()` Python function.
...
<.> By default, a Micronaut response uses `application/json` as Content-Type. We are returning `svg/xml`, so we have to set it.
<.> Use the `PygalModule` bean to create a `PygalModule.StackedBar` instance.
<.> Add graph title and data.
<.> Render the graph.
<.> Return the graph's `svg/xml` representation decoded as a string.
