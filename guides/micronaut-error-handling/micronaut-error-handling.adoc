common:header.adoc[]

common:requirements.adoc[]

common:completesolution.adoc[]

common:create-app.adoc[]

=== Global @Error

We want to display a custom `Not Found` page when the user attempts to access a URI that has no defined routes.

image::notfound.png[]

The views module provides support for view rendering on the server side and does so by rendering views on the I/O thread pool in order to avoid blocking the Netty event loop.

To use the view rendering features described in this section, add the following dependency on your classpath. Add the following dependency to your build file:

dependency:micronaut-views-velocity[groupId=io.micronaut.views]

The Micronaut framework ships out-of-the-box with support for https://velocity.apache.org/[Apache Velocity], https://www.thymeleaf.org/[Thymeleaf] or https://jknack.github.io/handlebars.java/[Handlebars]. In this guide, we use Apache Velocity.

Create a `notFound.vm` view:

resource:views/notFound.vm[]

Create a `NotFoundController`:

source:NotFoundController[]

callout:controller[arg0=/notfound]
<.> Inject an available https://micronaut-projects.github.io/micronaut-views/latest/api/[ViewRenderer] bean to render an HTML view.
<.> The Error declares which `HttpStatus` error code to handle (in this case 404). We declare the method as a global error handler due to `global = true`.
<.> If the request `Accept` HTTP header contains `text/html`, we respond with an HTML view.
<.> By default, we respond JSON.

=== Local @Error

common:validation.adoc[]

Then create a view to display a form:

image::createbook.png[]

resource:views/bookscreate.vm[]

common:serde-params.adoc[]

Create a controller to map the form submission:

source:BookController[tags=package|imports|clazz|create|save|createModelWithBlankValues|endOfFile]

callout:controller[arg0=/books]
<.> Use `@View` annotation to indicate the view name which should be used to render a view for the route.
<.> You can specify the HTTP verb that a controller action responds to. To respond to a GET request, use the `io.micronaut.http.annotation.Get` annotation.
<.> `@Consumes` annotation takes a `String[]` of supported media types for an incoming request.
<.> The `@Post` annotation maps the index method to all requests that use an HTTP POST
<.> Add `@Valid` to any method parameter which requires validation. We use a POJO to encapsulate the form submission.

Create the POJO encapsulating the submission:

source:CommandBookSave[]

callout:serdeable[]
<.> `title` is required and must not be blank.
<.> `pages` must be greater than 0.

When the form submission fails, we want to display the errors in the UI as the next image illustrates:

image::createbookserrors.png[]

An easy way to achieve it is to capture the `jakarta.validation.ConstraintViolationException` exception in a local `@Error` handler. Modify `BookController`:

source:BookController[tags=clazz|di|onSavedFailed|createModelWithBlankValues|endOfFile]

<.> Constructor injection
callout:on-error[]
callout:on-error-request[]

Create a `jakarta.inject.Singleton` to encapsulate the generation of a list of messages from a `Set` of `ConstraintViolation`:

source:MessageSource[]

== ExceptionHandler

Another mechanism to handle a global exception is to use an `ExceptionHandler`.

Modify the controller and add a method to throw an exception:

source:BookController[tags=clazz|stock|endOfFile]

callout:controller[arg0=/books]

source:OutOfStockException[]

Implement an `ExceptionHandler`, a generic hook for handling exceptions that occur during the execution of an HTTP request.

source:OutOfStockExceptionHandler[]

callout:singleton[]
<.> This bean loads if `OutOfStockException`, `ExceptionHandler` are available.
<.> Specify the `Throwable` to handle.
<.> Return 200 OK with a body of 0; no stock.

common:graal-with-plugins.adoc[]

:exclude-for-languages:groovy,python
After you run the native executable, execute a curl request:

[source, bash]
----
curl -H 'Accept: text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8' localhost:8080/foo
----

You should get a successful response.

[source, bash]
----
<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
    <title>Not Found</title>
</head>
<body>
<h1>NOT FOUND</h1>
....
----

:exclude-for-languages:

common:next.adoc[]

common:helpWithMicronaut.adoc[]
