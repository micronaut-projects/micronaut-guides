...
test:SessionAuthenticationTest[]

callout:micronaut-test[]
<.> Use `requests.with_context` to send HTTP requests to the running test server.
<.> Anonymous users can render the Home page.
<.> Invalid credentials redirect back to the failed login page.
<.> Valid credentials redirect back to the Home page.
<.> The same client keeps the session cookie and renders the authenticated Home page.
<.> The logout endpoint clears the session and redirects back to the Home page.
