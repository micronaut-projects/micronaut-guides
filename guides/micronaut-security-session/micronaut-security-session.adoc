...
Add Micronaut's security dependencies to your build:
...
By default, Micronaut's controllers produce JSON. Usually, you consume those endpoints with a mobile phone application, or a JavaScript front end (Angular, React, Vue.js, etc.). However, to keep this guide simple we will produce HTML in our controllers.
...
Add a dependency for Micronaut's https://docs.micronaut.io/latest/guide/index.html#views[Server Side View Rendering Module] and Velocity:
...
<2> Annotate with `io.micronaut.http.annotation.Controller` to designate the class as a Micronaut's controller.
