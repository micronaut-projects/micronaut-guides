...
If you use https://launch.micronaut.io[Micronaut Launch], select "Function Application for Serverless" as the application type, JDK version 17 or higher, and add the `graalvm`, `oracle-function`, and `oracle-cloud-httpclient-netty` features.
...
First, log in to your Oracle Cloud tenancy as an administrator, or a user with sufficient permissions to create the infrastructure described below.
...
Update your build script with the location where you will deploy the Docker container.
