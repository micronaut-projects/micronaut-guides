...
include::{commondir}/common-docker-push.adoc[]
...
include::{commondir}/common-install-graal-function.adoc[]
...
In your `build.gradle`, if the `nativeImage` block includes `--static` in the `args` list, replace it with `-H:+StaticExecutableWithDynamicLibC`. Also change the base image to `frolvlad/alpine-glibc:alpine-3.12` in the `dockerfileNative` block:
...
include::{commondir}/common-docker-push-native.adoc[]
