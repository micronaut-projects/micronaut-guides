...
[source,bash]
----
mn create-function-app --features=oracle-function example.micronaut.micronautguide --build=@build@ --lang=@lang@ --jdk=11
----

include::{commondir}/common-build-lang-arguments.adoc[]

If you use https://launch.micronaut.io[Micronaut Launch], select "Function Application for Serverless" as application type, JDK version 11 or higher, and add the `oracle-function` feature.
