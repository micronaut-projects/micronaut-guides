...
Micronaut Framework uses https://opentelemetry.io/[OpenTelemetry] to generate and export tracing data.
...
<1> Inject the `Tracer` bean into the class
<2> Creates a new span called "stock-counts"
<3> Adds a label, or tag, called "inventory.item" that will contain the value in `item`
...
<2> Same as `@SpanAttribute("order.item")` and `@SpanAttribute("count")`
